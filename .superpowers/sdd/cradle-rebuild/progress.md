...
### 2026-09-10 (late) — 6A remainder: native Save, round-trip fidelity, conflict, export — all walked green

- OFFLINE PURITY (§3.3): both committed payloads verified network-free — the only `http` occurrence in either file is the SVG namespace; the 4+2 Mermaid fixture is kept and labelled as code ("recorded as code"), never remote-rendered. Both documents' saving law (self-contained Save HTML copy, no network, 12 MB media cap) is carried by their own code.
- DEFECT FOUND AND REPAIRED (desktop, explicit): ⌘S reached `FileSurface.save()` TWICE per keystroke — TextEditor's CodeMirror `Mod-s` keymap AND the scroll pane's `onKeyDown` both dispatch it. On large documents the second write launched with the same base revision and landed as a conflict against the first write's own advance (error banner "The file changed", footer stuck at "Last reading"). Repair: a `savingRef` re-entry guard so a save already in flight is never re-entered; both dispatch paths retained (the pane-level one catches ⌘S when the editor does not hold focus).
- REPAIR (host, §3.3): the material iframe sandbox now includes `allow-downloads` — the documents' own "Save HTML copy" is a frame-local, network-free export the host must permit; the frame stays opaque-origin with no bridge authority. `material.mjs`'s exact-sandbox assertion updated accordingly; material scenario re-walked 26/26.
- WALK (`document-entry`, real bridge + real ctrl + isolated scratch ground seeded with the committed bytes): **17/17 PASS**. New evidence on top of the entry cell: (1) a desktop edit saves through Central with an advanced content-addressed revision; (2) the REAL 466,929-byte payload round-trips the desktop editor byte-exactly — after edit+revert the file's content hash returns to the original revision; (3) an external change behind a stale basis meets the structured conflict — current file shown beside the retained draft, nothing overwritten, and rebasing the draft lets the save land; (4) the die performs its own Save HTML copy inside the sandbox (download observed, .html filename); (5) an 0/1 edit saved through the desktop is what Central then holds, and after owner re-read the rendered document shows the saved writing. Receipt `walk/artifacts/document-entry.json` + new `dialogue-01-native-save` screenshot.
- Walk mechanics learned (recorded for the next walker): the FileSurface footer status is deliberately collapsed to a sliver until hovered (cradle.css pane law) — walks hover-then-read or use exact-match regex ("Unsaved" contains "saved"); after the conflict rebase button takes focus, the caret must return to the editor before ⌘S (the button sits outside the scroll pane that carries the save handler).
- Two transient boot failures (0/0 receipts) were orphaned dev walk services from prior runs racing the new spawn on ports 4173/4179; clean re-runs green. A concurrent desktop preview (port 4174, main worktree, started 23:02) belongs to another live session — left untouched (serialization law).
- OWNER SEAMS (re-checked 2026-09-10 ~23:00): ai-kit PR #278 (addressed delivery/placement/recurrence cores) still OPEN — 6B dispatch stays gated; Central PR #155 (placement/NOW-Day/migration/documents/reviewed receiving — the #150–#152 lane) now carries its implementation as an open PR — 6E inclusion stays gated until it lands. No desktop consumer work invented against unmerged seams.
- 6A standing after this cell: entry, payload fidelity, native Save/round-trip/conflict/recovery and portable export all have D/C walk evidence on the real payloads. Remaining named opens for full 6A closure: installed-candidate walk per §10 (serialized, owner present) and the Tauri `oi-material://` variant of the export path (walk covers the bridge transport; the shipped protocol handler exercises the same sandbox attributes).

### 2026-09-10 (late) — 6E gate readiness note (Central PR #155 inspected, not consumed)

- Central PR #155 (`agent/caw-temporal-documents-receiving`) now carries real implementation (not just workflows as at the earlier #220 comment): `ctrl/src/continuous_work/{placement,temporal,documents,receiving,migration,source,history,authority,extended}.rs` plus integration tests. Diff inspection names the owner operations the 6E consumer will bind when it lands: `central.receiving.submit|review|include|recover`, `central.document.create|mutate`, `central.day.ensure|lifecycle`, `central.now.allocate|lifecycle|read|obligations`, `central.migration.plan|apply|rollback|recover`, `central.work.policy|validate`, `central.time.policy`, `projectcentral.source.write`. Recorded so the next dispatch binds these verbatim instead of re-inventoried names. Desktop consumer work deliberately NOT started against the unmerged PR (no consumer-side invention); 6E opens when it merges and installs.

### 2026-09-10 (late) — 6B gate readiness note (ai-kit PR #278 inspected, not consumed)

- ai-kit PR #278 (`agent/caw-274-277-native-continuation`) still OPEN at ~23:55, mergeable, 41 files. Its own `docs/implementation/CAW-NATIVE-DELIVERY.md` states plainly it is an implementation cut, not closure of #274/#275/#276/#277 and not continuous-work acceptance. Diff inspection (doc + CLI surface) names the native operations the 6B desktop consumer will bind when the PR merges AND installs — recorded verbatim so the next dispatch binds them instead of re-invented names. Names read from an unmerged branch: re-verify at merge before binding.
- Provisioning (owner-only, one-time per session space): `aikit-session-space -C <cwd> encounter-agency-configure --agent-session <ref> --binding-json @binding.json [--expected-revision <current-revision>]` (the `EncounterAgencyBinding` in `crates/aikit-cli/src/encounter_agency.rs` — explicit active flag, revision, exact native identities, source basis, Actuation executable, permitted senders, packet-source disclosure, optional exact context admission; deliberately absent from the IPC request enum), `aikit-session-space -C <cwd> encounter-configure --provider-json @provider.json` (`protocol: "acp" | "pi-rpc"`; omitted defaults to ACP), `aikit-session-space -C <cwd> encounter-serve --socket <private-dir>/owner.sock`.
- Addressed dispatch (through the existing `aikit encounter --request-json @request.json --socket …`): request enum now carries `send`, `send-group`, `delivery`, `reconnect` beside existing `open`/`read`/`view`/permissions/human-interaction. Addressed turn: `{"action":"send","agent_session":…,"turn":{"delivery_ref":…,"sender":…,"expected_binding_revision":…,"packet":{"text":…,"source_refs":[…],"audience":[…]}}}`. Group: `send-group` with `recipients:[{agent_session, expected_binding_revision}]` — fanout NOT atomic, every recipient gets an independent durable result. Readback: `{"action":"delivery","agent_session":…,"delivery_ref":…}`; `submitted` is a transport ACK, `returned` means the native host observed a completed provider turn (not task success, not artifact receipt). Recovery: `aikit-session-space encounter-delivery-reconcile --agent-session <ref> --delivery-ref <ref> --expected-phase <dispatching|submitted|uncertain> --evidence-ref <native-ref>` records `reconciled-no-replay`.
- Binding/composition: `aikit compose --agent <ref> --world <ref> --agency-source <basis.json>` (basis = source_ref, revision, absolute path, `blake3:<digest>`); the agency binding runs the existing `actuation agency actualise <exact-staged-source.json> --json` with source-digest checks before/after. Authority gate: the admitted source must permit `action/aikit/encounter-send`; presence or a profile cannot confer it.
- Consumer-facing semantics to preserve: repeated exact `(AgentSession, delivery_ref)` returns its durable receipt and never resends (including after owner restart); one unresolved machine turn per session; a transport failure is not automatic retry permission; profile-less declared AgentSet members are declared participants but NOT live/authorised/defaults; `reconnect` loads the recorded native session identity and refuses contradictory identities; response events carry the `delivery_ref` only when their connection generation matches the pending delivery; Pi supports native attach only, not invented session-load.
- PR's own declared remaining web-code joins (so no mis-grading later): gateway ingest/connector delivery → addressed native session + filtered response egress is #275's unfinished core (gateway stream append is still not execution); Central #153 → AIKit #277 placement policy/allocation/write-validation operations deliberately NOT named ("no operation names were guessed from issue prose"); Central #150 → AIKit #276 native time policy/occurrence reading; Central #152 → headless Return receiving remains the Central owner join. Desktop 6B consumer work deliberately NOT started; 6B opens when the PR merges and installs.

### 2026-09-10 (late) — installed-candidate walk prepared: binding facts recorded, run gated on owner presence and serialization

- Installed baseline inspected (read-only): `oi` at `/Users/admin/.local/bin/oi` is `0.1.0-prelocal.2` (build `c187e29c…-dirty`, suite build record 2026-08-17, self-described `historical-unratified-prelocal-build-record`) — the binary whose registered Central owner predates `central.world`/`central.recognize`, which is why walks pin `OI_CENTRAL_CTRL_BIN`. Product catalogue (`oi products --json`) verified 2026-09-03 with Central `command_revision f66794a7…` `accepted-main` — older than the Sep 10 cargo-built `/Users/admin/.cargo/bin/ctrl` (0.1.0, built Sep 10 01:47) the walks use. The installed desktop does NOT carry wave 6; nothing of 6A is installed.
- Candidate to bind (§10): one coherent candidate from THIS branch (`agent/oi-wave6-document-entry` @ the wave-6 tip) through the O:I install/update/source-modality system — `oi dev build` / `oi dev install` for the desktop under the developer path, or `oi install` — recording the actual `oi` and every delegated owner executable's path/revision/digest (`oi products --json` at bind time), relevant contract versions, source basis and explicit overrides. The walk tests the exact dispatch route the desktop uses, not a fresher binary found elsewhere.
- Walk scope (folds in the two named 6A opens): the whole document-entry path on the INSTALLED candidate — blank-tab choices, real payload open, native Save/round-trip/conflict/recovery, and the Tauri `oi-material://` export variant of the documents' own Save HTML copy (the shipped protocol handler exercising the same sandbox attributes the bridge walk proved). Required owner executable bindings re-verified at walk time; no binary replacement under the walk — a newer owner revision starts a new candidate.
- GATING (law, restated so nobody runs it loose): serialized — never overlap with another session's candidate build/walk; owner present for the installed interaction evidence (D/C walk receipts stay what they are; the installed walk supplies the M-side interaction evidence; human EX remains its own grade). NOT scheduled yet — proposed to the owner through the NOW field return of this session; do not start it opportunistically.

### 2026-09-11 — GATING LAW CORRECTED: unmerged prerequisite PRs with real operations open their consumer cells on a pinned candidate cut

- Correction of this ledger's own earlier line ("6B opens when the PR merges and installs" / "no consumer invention before merge+install"). The owner dispatch on O:I #220 (2026-09-10 21:54Z) is explicit: "For unmerged prerequisites, test an explicitly pinned candidate cut, publish PR dependencies/merge order, and do not call it accepted main." The handoff's prohibition was against INVENTED operations and simulated replies — not against consuming an owner's real, tested operations at a recorded revision. Both gate PRs carry exactly that: implemented, CI-proven operations with named residue (ai-kit #278: one failing enforcement test + declared #274/#275 joins; Central #155: capability-catalogue reconciliation, functional work green at head). So the desktop consumer cells for 6B and 6E open NOW on pinned cuts; what still waits for merge+install is closure standing and any installed interaction evidence. Consumer bindings must record the exact candidate revision and never ship as if accepted main.

### 2026-09-11 — 6B cut 1: addressed dispatch walked green on the pinned ai-kit candidate (select-send 12/12)

- PINNED CANDIDATE (§10 binding, recorded): ai-kit `pr-278-candidate` = PR #278 head `3d23d1eefbb999b0a5058ed0b4ca98dd2575b632`, worktree `<scratchpad>/aikit-pr278`, cargo release build. Digests: `aikit` sha256 `a2b27296cea15e159168e867202c08a2dc8ef90a6d9c8e1044ccc339adc37830`, `aikit-session-space` sha256 `6d4ccbf708585e1fd4c8c3278e1cba926148fb156e7e86d72ee28981972ba7e1`. Delegated owners: Actuation `actuation 0.2.0` at `/Users/admin/.npm-global/bin/actuation` (its `agency actualise --json` accepted the staged agency fixture); Central walks keep `/Users/admin/.cargo/bin/ctrl`. Not accepted main; consumer standing = D/C on the pinned cut.
- DESKTOP DELTA (one cell: single addressed send + delivery readback; group/reconnect deliberately deferred): kernel `agency.rs` gains `send`/`delivery`/`health` request variants with the owner wire contract carried verbatim (`AddressedTurn{delivery_ref,sender,expected_binding_revision,packet{text,source_refs,audience}}`), the attachment gate now covers every named session, and owner refusals surface their code — `… (encounter.disclosure_denied)` — instead of a bare message. Panel: new `AddressedComposer` (distinct card BELOW the human composer in the Conversation plane — explicit machine turn, never a second chat box, never the human draft buffer) with sender/recipient/basis/source-refs fields, exact preview, owner-service health disclosure, and the delivery state machine preparing→dispatching/submitted→returned|failed|cancelled|reconciled with the honest "not task success, no recognition implied" note; duplicate receipts shown as held-not-resent; refusals shown verbatim. `client.ts` mirrors the contract + mints sender-side `delivery/desktop-…` refs. Delivery polling is view-visible-only and stops at terminal phase.
- WALK (`select-send`, new scenario alias `6b`; real bridge + real ctrl ground + PINNED aikit binaries + real Actuation admission + controlled provider `walk/fixtures/addressed-provider.py` (copied from the pinned PR's test fixture; FIXTURE_REPLY, D/C only)): **12/12 PASS**, receipt `walk/artifacts/select-send.json` + 3 screenshots. Proven: service health disclosed before composition; nothing dispatches until composed; exact preview; the UI dispatch landed on the owner, delivery settled `returned`; the reply reached the canonical transcript through the ordinary read path (as provider-attributed material — an addressed turn is NOT a user block, the owner says so in its transcript shapes); the provider log holds exactly ONE prompt carrying the native `Selected Agent/Agency/WorldBinding` header + exact packet text and NOT the composed-but-unsent human draft; the duplicate replay of the same delivery identity returned the durable receipt with NO second provider prompt; an undisclosed audience was refused with `encounter.disclosure_denied` shown verbatim and zero provider effect.
- WALK MECHANICS learned: provisioned agency/context sources must live at a lexically real path (`realpathSync`) — the owner refuses symlink-redirected agency sources and macOS `/var` is one (Linux CI never saw this). Owner provisioning sequence for a selectable addressed participant (all owner-side, per §10): stage agency source fixture → `encounter-agency-configure --binding-json` (runs `actuation agency actualise` with byte/digest checks) → `encounter-configure` provider → `encounter-start` (daemonises `encounter-serve` on the canonical AIKIT_HOME socket — the desktop's `encounter --request-json` then reaches it without socket plumbing). The desktop deliberately has NO binding read: configuration is absent from the owner's IPC by design, so recipient/basis fields are owner-validated inputs, not invented desktop state.
- REGRESSION: `context-draft` re-walked 12/12 on the new bundle WITH the pinned binaries (draft CAS/view/attach through the changed agency client — also first compatibility proof of the pinned candidate against existing desktop paths). Kernel unit tests 29/29. `encounter` scenario re-walk OWED: it requires the owner's real provider binding (`AIKIT_ACP_NATIVE_ARGV`), which this session does not hold and will not hunt for; the one at-risk selector (`.encounter textarea` first-match) is safe by construction since the addressed card renders after the human composer.
- NAMED OPENS for 6B closure: (1) send-group/reconnect UI cells on the same seam; (2) participant/formation enumeration from World participation (ai-kit #274 remaining caller work) to replace operator-supplied recipient/basis fields — until then the fields stay explicit and owner-validated; (3) delivery-ref semantic-ref registration for the Inspect plane; (4) ai-kit #278's own failing enforcement test + declared joins (owner lane); (5) the serialized installed-candidate walk.


### 2026-09-11 — 6E cut 1: reviewed receiving walked green on the pinned Central candidate (receive-include 11/11)

- PINNED CANDIDATE (§10 binding, recorded): Central PR #155 head moved past the earlier `e7e8479` to `b79232b7e31741ccc3305cf239b0fdae3ee027bf` ("merge verified maintenance standing repair") — the head the earlier tooling-selftest failure was repaired at. Worktree `<scratchpad>/central-pr155`, cargo release `ctrl --locked`. Digest: sha256 `65774b440b82c0727e69f5e10dd9b1dd1464c57dbc4666ef65583d94da6b59e6`. Not accepted main; consumer standing = D/C on the pinned cut.
- DESKTOP DELTA (one cell: the pending-Returns tray — list, read, exact-basis review, include; recover/replace-own deliberately deferred): kernel gains `KernelOp::Receiving` routing to `central.receiving.list|read`, `central.document.read`, `central.receiving.review|include` with inputs composed owner-verbatically and the owner's refusals surfaced verbatim; the host supplies the native credential through the protected process environment (`CENTRAL_NATIVE_TOKEN` inherited by the ctrl child) — the kernel never handles tokens. New `ReturnsTray` under each project branch in the World navigator: pending returns with real producer attribution, expandable detail (exact proposed operation + content, basis at arrival vs current document basis, external-edit flag, review record), "Accept current basis"/"Reject"/"Include" through the owner's revision-checked operations, and a manual refresh affordance for arrivals while the navigator sits open. When the bound Central owner does not expose receiving, the tray is QUIETLY ABSENT — the desktop never advertises a capability its owner cannot do (this is what keeps every standing scenario against main ctrl visually unchanged).
- WALK (`receive-include`, new scenario alias `6e`; real bridge + PINNED ctrl + a scratch ground seeded exactly as the pinned PR's own integration test seeds it — root policy sources + relations + two credentialed principals): **11/11 PASS**, receipt `walk/artifacts/receive-include.json` + 4 screenshots. Proven: a pending Return visible with real attribution before any human act; the exact proposed operation, content and basis revision shown before any decision; acceptance records the human reviewer on the exact reviewed basis; inclusion lands through the owner's revision-checked operation; the document then holds exactly one contribution with `author agent:walk / display_role Agent / reviewed_by human:walk` as distinct native facts and an advanced source revision; a second return accepted against a just-externally-edited basis is REFUSED with the owner's exact words ("reviewed target source changed") and changes no document bytes.
- OWNER FACTS learned (recorded so the next dispatch doesn't rediscover them): credential grants need far-future `expires_at_unix_seconds` (the PR test's 9999 is 1970 in real time — its harness pins now=100) and a bounded-length bearer token; project-scoped document sources live under `Work/<project>/`; after an external human edit the owner BLOCKS native contribution mutation until the retained revision is reconciled — the desktop's first scenario draft wrongly expected inclusion to proceed past an unreconciled external edit, and the owner's refusal is the correct behaviour, kept.
- REGRESSION: `navigator` 25/25 and `document-entry` 17/17 re-walked on the new bundle with the standard main-ctrl binding (tray quietly absent there); `context-draft` 12/12 after the 6B delta; kernel unit tests 29/29. The 6E kernel delta touches no existing adapter path.
- NAMED OPENS for 6E closure: (1) `central.receiving.recover` affordance for interrupted/uncertain inclusion; `replace-own` and reject flows in the tray; (2) returns rendered beside/around the OPEN document (§3.1 placement) rather than the project tray; (3) fixture-level targeting (Day positions/entry anchors) in the tray's proposed-operation display; (4) the Tauri host passing `CENTRAL_NATIVE_TOKEN` (host credential law, not kernel work); (5) Central #155's capability-catalogue reconciliation + merge (owner lane), then install; (6) the serialized installed-candidate walk covering both cuts.

### 2026-09-11 — 6 first vertical: one joined walk on the pinned cuts — document open → passage selection → addressed turn → attributable reply → reviewed Return (first-vertical 18/18)

- GATE RE-CHECK (per-session law): ai-kit #278 head UNMOVED at `3d23d1e`, Central #155 head UNMOVED at `b79232b` (fetched `refs/pull/*/head` fresh this session); the pinned binaries in `<scratchpad>/aikit-pr278-target` and `<scratchpad>/central-pr155-target` re-verified byte-exact against the ledger digests (`a2b27296…`, `6d4ccbf7…`, `65774b44…`). No re-pin, no rebuild, no re-walk owed before new cells.
- DESKTOP DELTA (seam extension only, no new stores): the context tray's selection dialog gains a second destination, "Address to the participant" — the same currency-validated composition (exact text, source ref, selection-time revision) that feeds the AIKit draft now routes to the conversation's addressed-request composer instead. Handoff survives the tab system's unmount-on-inactive law through a consumed-on-view pending slot in the composer module (presentation state only: never persisted, never auto-sent; the composer shows the composed source ref and quote, and sender/audience/basis stay explicit owner-validated inputs).
- WALK (`first-vertical`, new scenario alias `vertical`; joined provisioning: the receive-include Central ground + the select-send AIKit agency/provider ground in ONE scratch root; the Central agent principal is named `agent:editor-walk` — the same ref the addressed packet names as recipient, so the identity join across the two owner cuts is by construction): **18/18 PASS**, receipt `walk/artifacts/first-vertical.json` + 7 screenshots. Proven end to end: the native document (created through `central.document.create`, a real file under the project) opens through the ordinary navigator route and renders its own bytes; a selected passage composes an addressed request carrying the document's exact Central source ref and the selection-time revision; the pinned ai-kit cut dispatches it through real Actuation admission (the participant's packet-source disclosure was provisioned to admit the document's real source ref — a disclosure that does not admit it is refused, as the owner's gate did before the fixture was widened); the reply reaches the transcript as provider-attributed material with exactly one provider prompt, never the human draft; the reply text is then submitted VERBATIM as a Return by the same participant ref through the owner's real `central.receiving.submit` (owner-side credential, task_ref carries the delivery identity); the human reviews it in the tray — attribution "Agent — agent:editor-walk", the exact reply content, the basis revision identical to the one the selection recorded — accepts the exact current basis, includes, and the document ends holding the agent's reviewed reply as distinct native facts (`author_ref agent:editor-walk / display_role Agent / reviewed_by human:walk`) at an advanced revision with the proposed entry anchor in the real bytes.
- OWNER FACTS learned: a project-scoped native document source lives under `Work/<project>/ProjectCentral/agents/now/flows/<key>.json` — the project scope's prefix is `ProjectCentral` (continuous_work/source.rs), not the project root; the navigator expands it by default. The owner refuses an addressed send whose packet sources the participant never admitted — a composed-from-selection request must be covered by the binding's `allowed_packet_sources`, which is the privacy admission doing its job.

### 2026-09-11 — 6B cut 2: addressed group dispatch and recorded-session reconnect walked green (send-group-reconnect 19/19)

- DESKTOP DELTA (same seam, no new stores): kernel `agency.rs` gains the owner's `SendGroup` request (one delivery identity, explicit per-recipient `expected_binding_revision`, packet verbatim; the kernel's attachment gate now covers EVERY named recipient session before the owner sees the turn) and `Reconnect` (space/session/provider, with the kernel supplying `cwd` as `Open` does — presentation never passes paths). `AddressedComposer` gains an "Addressed group" composition: explicit recipient lines (session + basis) and an explicit audience field — the desktop has no binding read and no participant discovery, so both stay operator-typed and owner-validated; per-recipient results render each recipient's own phase or the owner's refusal verbatim, with the honest "individually durable dispatch; no automatic replay" line. `EncounterView` gains a reconnect control that only appears after the owner's own `encounter.resume_required` refusal discloses that a recorded native session exists.
- WALK (`send-group-reconnect`, new scenario alias `6b2`; two residents provisioned through real Actuation admissions with independent bases `rev/1`/`rev/2` and separate controlled providers): **19/19 PASS**, receipt `walk/artifacts/send-group-reconnect.json` + 4 screenshots. Proven: both recipients settle on their own durable results under ONE group delivery identity, each provider receiving exactly one prompt carrying its own recipient's native selected-Agent header; an audience omitting a recipient is refused whole-group before any transport effect by that participant's disclosure law (`encounter.disclosure_denied`); an audience with a ref beyond the recipient set is refused by the group-agreement code (`encounter.group_audience`); an undisclosed packet source refuses the whole group; repeating the exact group returns durable per-recipient receipts with no resend; after an owner RESTART a fresh open is refused with `encounter.resume_required`, the explicit reconnect loads the actually recorded native session identity (fixture `fixture-native-stable`, `session/load` in the provider log, the loaded session's own replayed material in the transcript — not a fresh session), and the group's delivery receipt is durable across the restart.
- Also exercised honestly: a group whose recipient has no resident native session is refused whole-group by the owner before any effect — the walk connects both participants before dispatch, which is the desktop flow the owner's law demands.

### 2026-09-11 — 6E cut 2: reject, uncertain-inclusion recovery, and Returns beside the open document (receive-recover 14/14)

- DESKTOP DELTA: kernel `ReceivingRequest::Recover` routes to the owner's `central.receiving.recover` (return identity + expected return revision only — the replayed intent is the owner's own recorded inclusion request, never re-authored); the tray and a new `DocumentReturns` strip gain "Recover inclusion" for `including`/`uncertain` returns and show the owner's recorded `last_error` beside the record; the proposed-operation display now shows the proposal's EXACT document anchor (`entry.add — entry entry:recover`, `field.append — field walk-field`) instead of the operation name alone. `DocumentReturns` renders the project's receiving field BESIDE THE OPEN DOCUMENT (under the source surface, the §3.1 reading-site placement) with the same owner operations as the tray — and is QUIETLY ABSENT when the bound owner does not expose receiving or nothing targets the open source.
- WALK (`receive-recover`, new scenario alias `6e2`; pinned ctrl ground as cut 1): **14/14 PASS**, receipt `walk/artifacts/receive-recover.json` + 5 screenshots. Proven: a return's exact field anchor shown beside the open document before any decision; rejection records the human reviewer beside the document, contributes nothing, and the owner refuses to include the rejected return afterwards ("inclusion requires the authenticated accepting reviewer or a new explicit review" — captured verbatim in the receipt; the desktop's own UI hiding the button is NOT the guard, the owner is); an accepted return whose inclusion the filesystem refuses mid-flight (document file made read-only — the owner's `source_safety::replace` refuses with PermissionDenied) lands `uncertain` with the owner's own record shown and ZERO document bytes changed; after the environment is restored, "Recover inclusion" replays the recorded intent and the return lands `included` with full producer/human attribution and an advanced revision; the project tray shows the same receiving field — one field, two registers.
- WALK MECHANICS learned (for the next dispatch): a project-scope ctrl Action run that is REFUSED exits non-zero with the structured refusal on stdout — walk helpers must parse `error.stdout` or they report a bare "Command failed" instead of the owner's words; the tray's stale-basis scenario in `receive-include` had a latent race (the detail renders before its basis read lands, so an external edit could land mid-read and the acceptance would be legitimately current — the owner was right to accept it) — repaired by waiting for the "Current document basis" line before the external edit; walk-scenario repair, not candidate behaviour.

### 2026-09-11 — regressions on the final bundle; branch state

- Final bundle regressions: `context-draft` 12/12 (ContextTray addressed-destination refactor), `select-send` 12/12 (AddressedComposer group addition — first run caught a strict-mode selector collision: the group preview reused `.encounter-addressed-preview`; renamed to `.encounter-addressed-group-preview`, component fix, not a walk relaxation), `receive-include` 11/11 (after the basis-read race repair above), `navigator` 25/25 and `document-entry` 17/17 on the standard main-ctrl binding (the document strip quietly absent there, as designed), `first-vertical` and `send-group-reconnect` re-walked on the final bundle after the rename. Kernel unit tests 29/29 after both kernel deltas.
- One class of regression FOUND AND FIXED this session belongs to the tooling, not the product: the receive-include stale-basis race above. One belongs to the product: the preview class collision. Both repaired and re-walked green.
- Branch `agent/oi-wave6-document-entry` commits this session: `d678d0e` (first vertical), `c7b3e22` (6B cut 2), `da9b6a3` (6E cut 2), plus the class rename and scenario repairs in the final bundle commit.
- NAMED OPENS carried (unchanged in kind): participant enumeration from World participation (ai-kit #274 caller work) still blocks replacing the operator-typed recipient/basis/audience fields; `replace-own` and the Tauri host passing `CENTRAL_NATIVE_TOKEN` remain open; Central #155 capability-catalogue reconciliation + merge and ai-kit #278's failing enforcement test + declared joins remain owner lanes; the encounter scenario re-walk still waits on the owner's real provider binding (`AIKIT_ACP_NATIVE_ARGV`); the SERIALIZED installed-candidate walk (folding in the Tauri `oi-material://` export) still needs scheduling with the owner — now covering BOTH pinned cuts' consumer surfaces.

### 2026-09-11 — Wave 7 gate re-check and owner-contract intake: publication/admission/withdrawal live in O:I's portable SharedField floor; source-Return stays Central's receiving seam

- GATE RE-CHECK (per-session law): ai-kit #278 head UNMOVED at `3d23d1e` (OPEN), Central #155 head UNMOVED at `b79232b` (OPEN, both fetched fresh via the GitHub PR API); the pinned binaries re-verified byte-exact against the ledger digests (`a2b27296…`, `6d4ccbf7…`, `65774b44…`) at `<scratchpad>/aikit-pr278-target/release/{aikit,aikit-session-space}` and `<scratchpad>/central-pr155-target/release/ctrl`. No re-pin, no rebuild, no re-walk owed before new cells.
- INTAKE (recorded verbatim from owner code so the next dispatch binds, never invents): (1) PUBLICATION/AUDIENCE — `oi.projection/v1` via the in-repo portable floor `shared-field/index.mjs` (merged main, O:I #154 SharedField adapter owner): `createProjection` carries `audience.visibility ∈ {public, unlisted, restricted, private}` + explicit `audience.refs[]`, `subject {ref, kind}`, `source {system, revision}`, `publisher_participant_ref`, non-empty `provenance[]`; the transport floor is `PROJECTION_CAPABILITIES = ["publish","resolve","fetch","subscribe"]`. (2) ADMISSION/QUARANTINE — `shared-field/admission.mjs`: `oi.contribution-ingress-receipt/v1` with `state ∈ {quarantined, admitted, rejected, withdrawn}` and a `payload_fingerprint`; `oi.admission/v1` decisions `disposition ∈ {admitted, rejected, withdrawn}` with `admission_actor_ref` and optional `visibility`/`audience_refs`; `oi.contribution-index-decision/v1` kept separate; client copies never drive admission (hosted law). (3) WITHDRAWAL — `withdrawProjection(previous, {published_at, reason})` → revision+1, `state:"withdrawn"`, representation `oi.withdrawal/v1`, `withdrawal.source_history_deleted:false`; receivers retain what they already admitted (`oi.projection-receipt/v1`, `authority.mode:"observed-read-only"`). (4) SOURCE-RETURN — the pinned Central cut's `central.receiving.submit|review|include|recover` + `central.document.read`, exactly as the 6E/vertical cells already bind.
- OWNER FACTS that shape Wave 7: NO durable publication/share operation exists on the pinned ai-kit cut — its only audience law is dispatch-time (packet `audience` + `allowed_senders`/`allowed_packet_sources` disclosure; `session_space_contribution.rs` is a suite-composition registration seam, not a share relation), and Central #155 has none. The HOSTED SharedField carrier (SpaceTimeDB module) still runs a public-field proof and REJECTS non-public content — audience-filtered private carriage is its declared later security slice — so this cell's two-participant carriage is mediated by the walk over the portable contracts, graded D/C on the pinned cut; the hosted carrier slice is a named open gate, never simulated. The pinned ctrl's receiving submit carries arbitrary non-reserved proposal keys verbatim into the owner record (reserved: `source_ref, document_id, expected_revision, request_id, project, actor_kind, author_ref, actor`), so shared-field lineage rides the proposal the owner already holds.

### 2026-09-11 — Wave 7 first vertical: two participants share one document's material on the pinned Central cut (shared-field-return 26/26)

- DESKTOP DELTA (presentation only, no new stores, no kernel change): the context tray's selection dialog gains a third destination, "Publish to the shared field" (offered for revision-carrying source selections) — the same currency-validated composition routes to a `SharedFieldMaterial` strip beside the open document, which composes a REAL `oi.projection/v1` through the in-repo owner floor (`createProjection` / `withdrawProjection` imported, not re-implemented; desktop `tsconfig` gains `allowJs` for that import) with explicit Publisher-participant and Audience-participants inputs and restricted default visibility; the floor's own laws refuse an unnamed publisher and a restricted projection with no audience refs, in the UI, before any envelope exists. The strip discloses the full envelope (`data-projection`) and offers "Withdraw projection" with a required reason. `DocumentReturns` renders a shared-field lineage line ("admitted contribution — projection <ref> rev N · withdrawn by the publisher, admitted material retained · admission <ref>") from owner-carried proposal keys only. New `DocumentContributions` strip renders the document's accepted source revision — producer, display role, human reviewer, entry anchor, and the advanced revision — beside the open document.
- WALK (`shared-field-return`, new scenario alias `7`; PINNED Central cut `b79232b`, host-supplied `CENTRAL_NATIVE_TOKEN`, never document JSON; the receiver participant `agent:reader-walk` holds ONLY `central.receiving.submit`): **26/26 PASS**, receipt `walk/artifacts/shared-field-return.json` + 4 screenshots. Proven end to end: the human selects a passage of the real native document and publishes it as a restricted projection naming exactly `agent:reader-walk`; the receiving field quarantines the arrival under a payload fingerprint and records its admission as its own actor (owner floor contracts, exercised by the walk as the receiving side); the publisher withdraws THROUGH THE UI — revision 2, `source_history_deleted:false`, and the receiver's admitted records demonstrably survive the withdrawal; the receiver source-Returns the ADMITTED material through `central.receiving.submit` with the shared-field lineage in the proposal; the human sees the admitted-and-withdrawn lineage beside the document before any decision, accepts the exact current basis, includes through the owner's revision-checked operation; the document ends holding the contribution with `author_ref agent:reader-walk / display_role Agent / reviewed_by human:walk` at an advanced revision, and all four material states — local document, published (withdrawn) projection, admitted return with lineage, accepted revision — are visible at once in the ordinary canvas. No second panel, no chat architecture.
- PRODUCT REPAIR FOUND BY THE WALK (one, seam-level): in context mode the editor's component-pick capture treated the new tool strips as document material — an armed hover swallowed the publish click and re-opened the selection dialog. `EditorChrome`'s `pick`/`onClickCapture` now exclude the tool-strip regions (`.document-returns`, `.shared-field-material`, `.document-contributions`, history, conflict): a click on tooling means the affordance it names, nothing else. Also repaired: `DocumentReturns` is now the document's arrival point whenever the owner exposes receiving — refreshable even at zero rows (a Return can arrive any moment after mount; the project tray must not be the only place that notices) — quietly absent as before on owners without receiving, so main-ctrl grounds are visually unchanged; `DocumentContributions` follows the same law once a return targets the source; the published-projection record survives the tab system's unmount-on-inactive law through the same consumed-on-view pending-slot pattern the addressed composer uses (presentation state only — never persisted, never re-sent).
- WALK MECHANICS learned: refusal assertions must `waitForFunction` on the alert text — the check raced React's flush and read the previous refusal; a returns strip mounted before any arrival needs no remount now, but the accepted-revisions strip is discovered on the next mount, so the walk re-opens the document after inclusion (which also exercises the pending-slot carry).

### 2026-09-11 — regressions on the Wave-7 bundle; branch state; binding note

- Regressions, all green on the final bundle: `first-vertical` 18/18, `receive-include` 11/11, `receive-recover` 14/14, `context-draft` 12/12 (pinned env: Central `b79232b` ctrl + ai-kit `3d23d1e` binaries + Actuation at `/Users/admin/.npm-global/bin/actuation`); `navigator` 25/25 and `document-entry` 17/17 on the standard main-ctrl binding (`/Users/admin/.cargo/bin/ctrl`, real-ground World map). Kernel unit tests not re-run: no kernel delta this session.
- BINDING NOTE recorded so it is not re-paid: running the main-ctrl scenarios under the PINNED ctrl (or the PATH `ctrl` at `/Users/admin/.local/bin/ctrl`, an older registered owner) degrades them — under the pinned cut `navigator` lost the flow-subject focus check (24/25, not investigated further because the recorded standard binding passes 25/25), and under the old PATH binary `central.world` itself degrades (0/0). The standard main-ctrl binding for `navigator`/`document-entry` is `/Users/admin/.cargo/bin/ctrl` with `OI_CENTRAL_ROOT` unset.
- Branch `agent/oi-wave6-document-entry` commit this session: the Wave-7 vertical (tray publish destination, SharedFieldMaterial, DocumentContributions, return lineage, EditorChrome tool-strip repair, scenario + receipts).
- NAMED OPENS for Wave 7 closure (in kind): the hosted SharedField carrier cannot yet carry audience-restricted/private material (its declared later security slice) — this vertical's carriage is D/C on the portable contracts at a recorded revision, and installed/publication-carriage evidence waits for that owner slice; participant enumeration from World participation (ai-kit #274 caller work) still blocks replacing the operator-typed publisher/audience fields; A2A/SharedField crossing actual installations remains ahead of the two-participant one-installation vertical; `replace-own` and the Tauri host passing `CENTRAL_NATIVE_TOKEN` remain open; Central #155 and ai-kit #278 merges + installs remain owner lanes; the SERIALIZED installed-candidate walk still needs scheduling with the owner — now covering BOTH pinned cuts' consumer surfaces plus this Wave-7 vertical's surface.
