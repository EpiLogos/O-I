...
- `agent/oi-desktop-p5-system-109` → `quarry/notes/agent-oi-desktop-p5-system-109.md` (six owners × seven axes; see corrected disposition a)
...
- `agent/oi-epi-nara-coordinate-parity` → `quarry/notes/agent-oi-epi-nara-coordinate-parity.md` (fog Nara/Epi; see corrected disposition b)
...
- `agent/ql-relational-field` → `quarry/notes/agent-ql-relational-field.md` (docs+CSV+skill, not desktop; see corrected disposition c)
...
**Disposition corrected by owner, 2026-09-06 — no outstanding design decision here**
- (a) System/settings is already contracted by `docs/OI-DESKTOP-CRADLE-REBUILD-WAYFINDER.md` U1.6 (APP-SPEC §10 + §17.9). Execute it in the desktop programme: native six-product configuration, authored/effective/active distinctions, and a real owner-mediated configuration change. The previous “no home” assertion was stale.
- (b) Nara daily canvas remains deferred. Current desktop foundations must preserve selected disclosure, private source boundaries and native Action authority; those existing foundation obligations do not require building Nara.
- (c) Do not create another relation sheet. The six product `ProjectCentral/user/capability-matrix.csv` files now carry the old field as the `suite-relations` view under Central's `docs/CAPABILITY-MATRIX-PROTOCOL.md`. Read-only comparison against `ca78ec4:data/ql-relational-field.csv` found all 144 original relation identities and exact preservation of `ql`, `coverage`, `cf_view`, `seam`, `defined_in`, and `tracked_by`: missing identities = ∅; differing fields = ∅. These are concurrent, unpublished documentation changes; their owning task publishes them. No competing CSV or relational-development skill is to be resurrected from the retired branch.
...
| OI-ASTRA pinned product reality / continuation boundary | recorded 2026-09-06 | Latest AIKit aa4abc3 CI, pre-local and persistent-surface workflows all GREEN. Native Actuation detection from a clean ordinary clone at the actual suite pin faa5b79 discloses catalog_revision=1 with detected {claude-code,codex,gemini,gemini-antigravity,pi,hermes,hermes-acp,grok-bot,ollama,openclaw,kimi}, zcode absent. This supersedes earlier assumption that catalog r2 was proved against pins: r2 exists only in local unpushed 05a677b beneath active Actuation owner edits. No Actuation repair/push; owner release dependency filed at Actuation #1 comment 5559451933. Native real personal adoption preview enumerates the exact twenty skills at Control/user/skills, review e1b34c48d5d7dd77078cec61d0fbf390dc4f61fbbcdaf60e71840e4a84b69d31; applied=false, full preview kept locally, summary receipted. Live Central inspection still has the original twenty machine-staged copies plus personal central-ground-keeping (21 active, 0 retired, 0 unresolved); no real mutation. The exact seventeen dangling external names remain unchanged; published candidate main daa660c versus local ceb03ce still differ, with no lock authority for these names; source-owner question left pending. O:I #191 comment 5559452026 records the published cutover, fresh Codex proof, Pi OAuth failure, and remaining physical/Method/actor acceptance. Desktop work remains isolated. |

## 2026-09-06 — stale decisions resolved; published product convergence checked

System is already U1.6; Nara remains deferred; old relation data exactly survives in the concurrent canonical matrix work (disposition corrected above). No design or live desktop files changed. AIKit 9a7402e passed native `adopt_command`, `control_ground_source`, and `session_space_binary_project_context` suites; no failures/skips. Published CI, Pre-local, Native Skills and Persistent Surface lanes all green.

Actuation published 7c59dbc during verification. Exact published checkout returns catalog r2; native `aikit project bind` then `compose` in `/tmp/astra-r2-compose-proof` consumes that detector through PATH. Output explicitly has no Central profile, no model selection, no composed inputs: this proves detector intake, not completed actor bootstrap. Evidence: `astra-evidence/compose-r2-intake.json`. All 87 Actuation tests pass locally, but its Native CLI CI fails the real-machine test requiring claude-code on a clean GitHub runner; other release lanes are also red. Filed owner report https://github.com/EpiLogos/Actuation/issues/1#issuecomment-5559665253. No repair of the live owner checkout.

AIKit repin is prepared locally. `verify-mainline-snapshot.py --live` currently refuses the old Actuation pin because its main moved; the newly published Actuation cut is not accepted while its release gate is red. No push of this candidate. K2 remains complete; K3/K4 personal-machine migration remains unperformed, with the 17 unresolved external links and original real-Control read-only restriction unchanged.

## 2026-09-06 — owner-authorised real K3/K4 cutover

Owner explicitly authorised real Control mutation and local on-disk source adoption in this task. The previous read-only restriction and question about choosing a published Epi revision no longer apply to this migration.

Completed on the real machine: all 20 former `~/.agents/skills` directory masters adopted with provenance into `~/Central/Control/user/skills`; archived superpowers adopted there with 15 retired standings; all 20 misplaced machine-scope copies compared byte/mode-for-byte/mode and moved into AIKit recovery storage. The 17 old Epi symlinks are backed by their existing relocated local source directories (16 in Epi-Logos-C-Experiments; vault-compiler in quaternal-logic-plugin/skills). No downloaded-source trust gate applied. Codex's on-disk wayfinder was retained in personal Control instead of overwriting it with the different selected upstream version; both source versions and the upstream orientation remain preserved.

AIKit now reconciles mixed harness roots through its native reversible adoption Procedure. All 45 selected/generated skills are linked through the stable personal generation in `~/.agents/skills`, `~/.claude/skills`, and `~/.codex/skills`. For this migrated set: missing = ∅, wrong links = ∅, payload/mode mismatches = ∅, retired present = ∅; all 17 migrated broken links resolve. Existing unrelated harness material stays in place, including a pre-existing broken nested m-dev link outside this migration. The canonical `.agents/skills` root has no authored Skill directories and discovery recognises it as generated. Undo material lives under `~/.aikit/state/adoption-undo` and `state/migration-undo`; it is not registered as a source. Set proof: `astra-evidence/real-migration-cutover-proof.json`; authored standing and exact cradle hashes: `real-migration-final-ground-proof.json`.

Fresh real Codex read brandkit, apply-cmea, epi-logos-vault-compiler and cradle-execution from `~/.codex/skills`; its catalogue excluded brainstorming and using-superpowers. Commands and final testimony: `real-migration-fresh-codex.json`; complete local transcript: `/tmp/astra-real-migration/fresh-codex.json`. All three cradle copies match the native repo SKILL.md SHA256. `oi skills sync` completed with zero conflicts and native AIKit pickup: `real-migration-guardian-sync.txt`. This closes real K3 and K4 acceptance; it does not claim the separate full operative human UX or composed actor bootstrap is complete.

Product change: local-directory promotion accepts local revisions without an extra trust flag; downloaded Git revisions retain explicit review choices. Existing Control retirement remains separate. Real source lifecycle, retirement/project routing, mixed-root recovery/undo, stale-source refusal, generation discovery tests and workspace Clippy passed; release installed at `~/.cargo/bin/aikit`. An owned generated Rust incremental cache exhausted disk during the final follow-up check; only that reproducible cache was removed. No authored or recovery content removed.

Final native product pin: AIKit `bc7c1588210690ba3696b5eef6c86226f9a89deb`, including source-identity preservation in projection receipts. The mixed-root end-to-end test asserts actual source CapsuleIds; final test, Clippy and release rebuild all pass. Previous full migration cut 806627a has all CI lanes green; the small receipt-identity follow-up has local verification and newly queued CI. Actuation `2b458afdf284c5ece368a51340c64db4fe758486` resolves its runner gate and is green; Workcell `b9cd5e02b862b5f4c8793ec96cfd32611f068522` changes only CI packaging/obsolete branch workflow, with both verification lanes green. Suite live equality is rechecked before push.
