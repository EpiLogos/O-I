...
| 10 | same refs through desktop/web, reconnect, restart | both | web + restart: §9; desktop: the Lane C step 5 unit (`desktop/cradle/walk/artifacts/shared-field-hosted.json`, 38/38 on the real host): the graph reading carries the hosted field as a third owner input (7 hosted nodes = 7 entries, 9 edges = 9 relations, `native_owner: shared-field`, refs verbatim), the Knowledge surface opens `world:central:project:O-I/wiki:node:project-root/o-i` to its Projection reading (projection ref, projection revision, source revision, target), an unbound target reads `unavailable` rather than error, and a publish from the document strip lands as a hosted row the same kernel then reads back; the same ref then renders on the host's Explore |
