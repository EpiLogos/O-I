...
/** Exact CSS vocabulary available to explicit runtime imports. Optional
 * workbench and syntax roles belong even when a minimal theme omits them. */
export function themeImportRules(bundledThemes = []) {
  const minimal = themeVariables(convertTheme({colors:{'editor.background':'#ffffff'}}, {id:'rules',name:'Rules',appearance:'light',source:{}}));
  return {
    roles: [...new Set([...Object.keys(minimal), ...Object.values(CHROME_KEYS).flat(), ...Object.keys(SYNTAX_REPRESENTATIVES), ...Object.values(TERMINAL_ROLES), ...ANSI_ROLE_NAMES.map(name => `--oi-terminal-ansi-${name}`)])].sort(),
    shadow_values: Object.fromEntries(Object.keys(SHADOWS.light).map(role => [role, [SHADOWS.light[role],SHADOWS.dark[role]]])),
    bundled_themes: bundledThemes.map(theme => ({id:theme.id,appearance:theme.appearance})),
  };
}

export function emitImportRules(docs) {
  return `${JSON.stringify(themeImportRules(docs), null, 2)}\n`;
}

/** One `.oi-desktop[data-oi-theme="<id>"]` block per theme. */
