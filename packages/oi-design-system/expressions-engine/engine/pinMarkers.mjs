...
class PinMarkerLayer {
  ...
  update(points, activeId, ghost, isLight, _camera) {
  ...
  rebuildDots(points, activeId, ghost) {
    ...
    for (const p of points) {
      ...
      alphas[i] = p.active ? isSelected ? 1 : 1 : 0.35;
  ...
  rebuildVolumes(points, activeId, ghost) {
    this.clearVolumeGroup();
    const items = [];
    const active = activeId ? points.find((p) => p.id === activeId) : void 0;
    if (active) {
      items.push({
        x: active.x,
        y: active.y,
  ...
  buildVolumeFor(item) {
    ...
    wrap.position.set(item.x, item.y, 0);
...
function buildUnitCircleGeometry(segments) {
...
function buildModeTickGeometry(radius, mode) {
  const pts = [];
  if (mode === "vortex") {
    const arms = 4;
    const arcSpan = 0.5;
    ...
    for (let i = 0; i < arms; i++) {
      ...
      for (let s = 0; s < steps; s++) {
        ...
        pts.push(
          Math.cos(t0) * radius * 1.08,
          Math.sin(t0) * radius * 1.08,
          0,
          Math.cos(t1) * radius * 1.08,
          Math.sin(t1) * radius * 1.08,
          0
        );
      }
