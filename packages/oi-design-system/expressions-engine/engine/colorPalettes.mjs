...
const COLOR_DISTRIBUTION_MODES = [
  ...
  },
  {
    id: "angularSweep",
    label: "Conic Sweep",
    shortDesc: "Rotary chromatic sweep revolving 360\xB0 around the glyph center",
    iconName: "RotateCw"
  },
  {
    id: "velocityThermal",
...
const BACKGROUND_ATMOSPHERE_MODES = [
  {
    id: "ambientGlow",
...
function computeBackgroundCSS(options) {
  ...
  if (mode === "vignette") {
    if (isLight) {
      const edge2 = `rgb(${Math.max(0, Math.floor(r * 0.88))}, ${Math.max(0, Math.floor(g * 0.88))}, ${Math.max(0, Math.floor(b * 0.88))})`;
      return `radial-gradient(circle at 50% 50%, ${baseBg} 35%, ${edge2} 100%)`;
    } else {
      const edge2 = `rgb(${Math.max(0, Math.floor(r * 0.35))}, ${Math.max(0, Math.floor(g * 0.35))}, ${Math.max(0, Math.floor(b * 0.35))})`;
      return `radial-gradient(circle at 50% 50%, ${baseBg} 25%, ${edge2} 100%)`;
    }
  }
  ...
  if (mode === "adaptive") {
    ...
    const centerGlow2 = hexToRgbaStr(primaryGlow, alphaPrim);
    const midGlow = hexToRgbaStr(accentGlow, alphaAcc);
    const edge2 = isLight ? `rgb(${Math.max(0, Math.floor(r * 0.92))}, ${Math.max(0, Math.floor(g * 0.92))}, ${Math.max(0, Math.floor(b * 0.92))})` : `rgb(${Math.max(0, Math.floor(r * 0.4))}, ${Math.max(0, Math.floor(g * 0.4))}, ${Math.max(0, Math.floor(b * 0.4))})`;
    return `radial-gradient(ellipse 75% 70% at 50% 50%, ${centerGlow2} 0%, ${midGlow} 40%, ${baseBg} 75%, ${edge2} 100%)`;
  }
  const alphaCenter = isLight ? intensity * 0.15 : intensity * 0.28;
  const centerGlow = hexToRgbaStr(primaryGlow, alphaCenter);
  const edge = isLight ? `rgb(${Math.max(0, Math.floor(r * 0.9))}, ${Math.max(0, Math.floor(g * 0.9))}, ${Math.max(0, Math.floor(b * 0.9))})` : `rgb(${Math.max(0, Math.floor(r * 0.3))}, ${Math.max(0, Math.floor(g * 0.3))}, ${Math.max(0, Math.floor(b * 0.3))})`;
