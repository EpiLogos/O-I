...
fn package_envelope_never_replaces_native_contribution_or_action_identity() {
    let manifest = parse_manifest(&format!(
        r#"{{
          ...
          "package_ref":"package:factory-build",
          "version":"1.0.0",
          "source":{{"kind":"git","locator":"factory","revision":"abc"}},
          "compatibility":[],"permissions":["network"],"effects":[],
          "contributions":[{{
            "contribution_ref":"factory.surface/build",
            "target_product":"software-factory",
            "target_contract":"factory.surface/v1",
            "minimum_contract_version":"1.0.0",
            "artifact":"build-surface",
            "permissions":[],"effects":[],
            "native_verification":{{"operation":"factory.verify","evidence_format":"json"}}
          }}]
        }}"#
    ))
    .unwrap();
...
fn selection_propagates_only_the_stable_subject_ref() {
    ...
    assert!(!json.to_ascii_lowercase().contains("context"));
}

#[test]
fn action_availability_capability_grant_and_authority_are_separate() {
    let binding = ready_contribution().actions.remove(0);
