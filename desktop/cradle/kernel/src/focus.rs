...
//! Ported KEEP-RE-EARN from `desktop/core/src/focus.rs`, essentially
//! as-is: same semantics, re-proven by the U0.4 walk, not rewritten.
//!
//! The whole application shares one notion of *what we are currently
//! talking about*: a stable subject plus its surrounding relations (current
//! World, Project, Journey, Agency encounter). Exactly one current focus
//! relation exists kernel-wide (03 §B); focus change propagates as an event
//! every surface may consume, and no component copies selection state
//! (02 §5, §7).
//!
//! Refs stay opaque: the kernel never re-owns another product's nouns and
//! never infers a relation from a ref's kind string (02 §9.3). Resolving
//! the surrounding relations from a World reading is the owning service's
//! act; this module only holds the relation honestly.

use crate::refs::SemanticRef;
use serde::{Deserialize, Serialize};
use std::fmt;

/// Schema of the focus relation carried by `KernelEvent::FocusChanged` and
/// the kernel snapshot.
...
focus_relation! {
    /// The one current subject (02 §7): source, knowledge node, agent, run,
...
/// Journey and Agency encounter. Absent relations are absent — an
/// observation, never fabricated (02 §10); the empty relation is state
/// `B0 No focus` (03 §B).
...
impl GlobalFocus {
    ...
    /// The one current Project relation exactly as the kernel holds it.
...
mod tests {
    ...
    fn a_ref_without_an_identifier_is_not_a_subject() {
        let mut focus = GlobalFocus::unfocused();
        let error = focus.focus_subject(reference("", "source")).unwrap_err();
