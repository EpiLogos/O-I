...
impl Ground {
    fn new() -> Self {
        ...
        client
            .run(
                "projectcentral.init",
                json!({"project":"Editor", "project_id":"editor-integration"}),
            )
            .unwrap();
        ...
        .unwrap();
        let horizon = client
            .run("projectcentral.change.horizon", json!({"project":"Editor"}))
            .unwrap();
        let source = horizon["sources"]
            .as_array()
            .unwrap()
            .iter()
            .find(|s| s["binding"]["path"] == "ProjectCentral/user/Document with spaces.md")
            .unwrap()["binding"]["ref"]
            .as_str()
            .unwrap()
            .to_owned();
    ...
    fn open(&self, kernel: &mut Kernel) {
        kernel
            .apply(KernelOp::SourceOpen {
                project: Some("Editor".into()),
...
fn real_cas_refuses_external_revision_and_preserves_both_sides() {
    ...
    let mut kernel = Kernel::new(ground.client.clone());
    ground.open(&mut kernel);
    let base = kernel.snapshot().buffers[&ground.source]
        .base_revision
        .clone();
    ...
    let KernelOpResult::SourceSaveFailed { buffer, failure } = outcome.result else {
        panic!("CAS must refuse")
    };
    assert!(
        matches!(failure, SourceWriteFailure::RevisionConflict { expected, current, .. } if expected == base && current != expected)
    );
    assert_eq!(buffer.content, "Human buffer with \"quotes\"\n");
    assert_eq!(
        buffer.conflict.unwrap().canonical_content,
        "Externally changed canonical text\n"
    );
    assert_eq!(
        fs::read_to_string(&ground.file).unwrap(),
        "Externally changed canonical text\n"
    );
    kernel
        .apply(KernelOp::SourceReread {
            project: None,
            source_ref: ground.source.clone(),
        })
        .unwrap();
    ...
    assert!(matches!(
        saved.result,
        KernelOpResult::SourceSaved { changed: true, .. }
    ));
    assert_eq!(
        fs::read_to_string(&ground.file).unwrap(),
        "Human buffer with \"quotes\"\n"
    );
}
...
fn changed_project_selection_cannot_reroute_an_open_buffers_save() {
    ...
    assert_eq!(
        fs::read_to_string(&ground.file).unwrap(),
        "Save to the original Project\n"
    );
    assert_eq!(
        fs::read_dir(ground.root.join("Work/Other"))
            .unwrap()
            .count(),
        0
    );
...
fn genuinely_missing_owner_is_unavailable_without_fabricated_sources() {
    ...
    assert!(matches!(
        ...
        oi_cradle_kernel::world::ListingAvailability::Unavailable { .. }
    ));
...
fn history_is_durable_owner_state_and_rechecks_current_retrieval_gate() {
    ...
    let read = |kernel: &mut Kernel| {
        let result = kernel
            .apply(KernelOp::SourceHistory {
                source_ref: ground.source.clone(),
            })
            .unwrap();
        assert!(result.receipts.is_empty());
        let KernelOpResult::SourceHistory { history } = result.result else {
            panic!("history")
        };
        history
    };
    ...
    fs::write(
        ground
            .root
            .join("Work/Editor/ProjectCentral/user/.no-agent-retrieval"),
        "",
    )
    .unwrap();
    assert!(restarted
...
fn browsing_other_projects_preserves_open_source_focus_and_dirty_buffer() {
    ...
    kernel
        .apply(KernelOp::SurfaceOpen {
            surface_id: "document".into(),
            kind: "source".into(),
            source_ref: Some(ground.source.clone()),
            title: "Document".into(),
        })
        .unwrap();
    kernel
        .apply(KernelOp::SurfaceFocus {
            surface_id: "document".into(),
        })
        .unwrap();
    ...
    assert!(!serde_json::to_string(&outcome.receipts)
        .unwrap()
...
fn real_aikit_wiki_read_search_focus_and_failed_use_do_not_forge_authority() {
    ...
    let ground = Ground::new();
    let wiki: serde_json::Value = serde_json::from_str(
        &fs::read_to_string(
            ground
                .root
                .join("Work/Editor/ProjectCentral/agents/wiki/wiki.json"),
        )
        .unwrap(),
    )
    .unwrap();
    let node = wiki["objects"]
        .as_array()
        .unwrap()
        .iter()
        .find(|o| o["object"] == "space")
        .unwrap();
    ...
    ground.edit(
        &mut kernel,
        "Original source draft survives native knowledge navigation",
    );
    let initial = kernel.snapshot().focus.clone();
    let op = |request| KernelOp::Knowledge {
        fresh: false,
        project: Some("Editor".into()),
        request,
    };
    assert!(
        kernel
            .apply(op(Request::Use {
                address: address.clone()
            }))
            .is_err(),
        "display or unread references cannot record use"
    );
    assert!(kernel
        .apply(KernelOp::SurfaceOpen {
            surface_id: "unvalidated".into(),
            kind: "knowledge".into(),
            source_ref: Some(reference.clone()),
            title: "unvalidated".into()
        })
        .is_err());
    let outcome = kernel
        .apply(op(Request::Search {
            query: node["title"].as_str().unwrap().into(),
        }))
        .unwrap();
    let KernelOpResult::Knowledge { data } = outcome.result else {
        panic!("native search result")
    };
    assert!(data["hits"]
        .as_array()
        .unwrap()
        .iter()
        .any(|hit| hit["resource"] == reference));
    assert_eq!(
        kernel.snapshot().focus,
        initial,
        "querying never moves semantic focus"
    );
    let outcome = kernel
        .apply(op(Request::Read {
            address: address.clone(),
        }))
        .unwrap();
    let KernelOpResult::Knowledge { data } = outcome.result else {
        panic!("native reading")
    };
    assert_eq!(data["resource"], reference);
    assert_eq!(data["revision"], node["revision"].to_string());
    kernel
        .apply(KernelOp::SurfaceOpen {
            surface_id: "wiki".into(),
            kind: "knowledge".into(),
            source_ref: Some(reference.clone()),
            title: "Editor wiki".into(),
        })
        .unwrap();
    ...
    kernel
        ...
        .unwrap();
    assert!(
        kernel.snapshot().focus.project.is_some(),
        "Central declares the project associated with its wiki space"
    );
    assert_eq!(
        kernel.snapshot().focus.subject_ref().unwrap().ref_id,
        reference
    );
    assert_eq!(
        kernel.snapshot().focus.subject_ref().unwrap().native_owner,
        "ai-kit"
    );
    assert_eq!(
        kernel.snapshot().buffers[&ground.source].content,
        "Original source draft survives native knowledge navigation"
    );
    let closed = kernel
    ...
    assert!(
        close.to_string().contains("ai-kit"),
        "closing a wiki carries its real native owner"
    );
    assert!(
        !close.to_string().contains("central.projectcentral"),
        "wiki close cannot be relabelled as a Central source event"
    );
    assert!(kernel
        .apply(op(Request::Read {
            address: Address::Wiki("wiki:node:missing".into())
        }))
        .is_err());
    assert!(kernel
        .apply(op(Request::Use {
...
fn native_session_space_discovery_is_project_scoped_and_never_moves_work() {
    ...
    let main_executable = std::env::var_os("OI_AIKIT_BIN")
        .map(PathBuf::from)
        .unwrap_or_else(|| "aikit".into());
    let bind = Command::new(main_executable)
        .env("AIKIT_HOME", &home)
        .current_dir(&cwd)
        .args([
            "--json",
    ...
    assert_eq!(
        binding["project"], "editor-integration",
        "AIKit resolves the exact real Central ProjectRef"
    );
    ...
    assert!(
        native(&["discover", "--project", "editor-integration"])
            .as_array()
            .unwrap()
            .is_empty(),
        "unbound native space is not a project encounter"
    );
    ...
    let mut kernel = Kernel::with_agency(
        ground.client.clone(),
        oi_cradle_kernel::agency::Client::with(executable, Some(home)),
    );
    ground.open(&mut kernel);
    ground.edit(&mut kernel, "Keep the original source while reading agency");
    let before = kernel.snapshot();
    let sequence = kernel.event_log().len();
    let result = kernel
        .apply(KernelOp::AgencyRead {
            project: "Editor".into(),
        })
        .unwrap();
    let KernelOpResult::AgencyReading {
        project_ref,
        spaces,
        ..
    } = result.result
    else {
        panic!("native agency reading")
    };
    assert_eq!(project_ref, "editor-integration");
    assert_eq!(spaces, expected);
    assert!(result.receipts.is_empty());
    assert_eq!(kernel.event_log().len(), sequence);
    assert_eq!(kernel.snapshot().focus, before.focus);
    assert_eq!(kernel.snapshot().buffers, before.buffers);
    assert!(kernel
        .apply(KernelOp::AgencyRead {
    ...
    assert!(
        kernel
            ...
            .is_err(),
        "ordinary directory cannot mint a ProjectRef"
    );
}
...
fn native_files_are_read_without_adoption_and_focus_never_reowns_source_drafts() {
    ...
    let KernelOpResult::DirectoryRead { directory } = kernel
        ...
        panic!("Native directory reading")
    };
    ...
    assert!(
        kernel
            .apply(KernelOp::SurfaceOpen {
                surface_id: "file-tab".into(),
                kind: "file".into(),
                source_ref: Some(location.ref_id.clone()),
                title: "Native".into()
            })
            .is_err(),
        "Unresolved path references have no focus authority"
    );
    let KernelOpResult::FileRead { reading } = kernel
        .apply(KernelOp::FileRead {
            location: location.clone(),
        })
        .unwrap()
        .result
    else {
        panic!("Native file reading")
    };
    ...
    assert_eq!(
        kernel.snapshot().focus,
        before,
        "Reading filesystem data alone does not move focus"
    );
    kernel
        .apply(KernelOp::SurfaceOpen {
            surface_id: "file-tab".into(),
    ...
    assert_eq!(
        snapshot.focus.subject.as_ref().unwrap().ref_id(),
        location.ref_id
    );
    assert_eq!(snapshot.focus.subject.as_ref().unwrap().kind(), "file");
    assert_eq!(
        snapshot.buffers[&ground.source].content,
        "Existing authored draft stays here\n"
    );
...
fn native_file_context_comes_from_central_manifest_and_cannot_be_rebound_to_another_root() {
    let ground = Ground::new();
    fs::write(
        ground.root.join("Work/Editor/ordinary.rs"),
        "fn main() {}\n",
    )
    .unwrap();
    let mut kernel = Kernel::new(ground.client.clone());
    let KernelOpResult::DirectoryRead { directory } = kernel
        .apply(KernelOp::FilesList {
            path: "Work/Editor".into(),
            fresh: None,
        })
        .unwrap()
        .result
    else {
        panic!("Native listing")
    };
    let location = directory
        .entries
        .iter()
        .find(|e| e.name == "ordinary.rs")
        .unwrap()
        .location
        .clone();
    let KernelOpResult::FileRead { reading } = kernel
    ...
    assert_eq!(
        reading.project.unwrap().project_ref.as_deref(),
        Some("editor-integration")
    );
    kernel
        .apply(KernelOp::SurfaceOpen {
            surface_id: "file".into(),
            kind: "file".into(),
            source_ref: Some(location.ref_id.clone()),
            title: "ordinary.rs".into(),
        })
        .unwrap();
    kernel
        .apply(KernelOp::SurfaceFocus {
...
fn ordinary_file_owner_cas_history_and_restore_through_consumer() {
    ...
    let ground = Ground::new();
    let path = ground.root.join("Work/Other/ordinary.md");
    fs::write(&path, "original\n").unwrap();
    let directory = files::list(&ground.client, "Work/Other").unwrap();
    let location = directory
        .entries
        .iter()
        .find(|entry| entry.name == "ordinary.md")
        .unwrap()
        .location
        .clone();
    let initial = files::read(&ground.client, &location).unwrap();
    assert_eq!(
        initial.operations.as_ref().unwrap()["write"]["available"],
        true
    );
    let mut kernel = Kernel::new(ground.client.clone());
    let mut run = |request| {
        let result = kernel
            ...
            .unwrap();
        let KernelOpResult::FileOperation { data } = result.result else {
            panic!("native operation")
        };
        data
    };
    let written = run(Request::Write {
        expected_revision: initial.revision.clone(),
        content: "edited\n".into(),
    });
    assert_eq!(written["outcome"], "written");
    assert_eq!(fs::read_to_string(&path).unwrap(), "edited\n");
    let stale = run(Request::Write {
        expected_revision: initial.revision.clone(),
        content: "stale draft\n".into(),
    });
    assert_eq!(stale["outcome"], "conflict");
    assert_eq!(stale["current"]["content"], "edited\n");
    let history = run(Request::History {
        limit: Some(10),
        before: None,
    });
    assert_eq!(history["entries"].as_array().unwrap().len(), 1);
    let current = written["revision"].as_str().unwrap();
    let preview = run(Request::RecoveryPreview {
        expected_revision: current.into(),
