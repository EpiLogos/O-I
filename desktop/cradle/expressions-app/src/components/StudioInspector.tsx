          ...
          <div>
            ...
            {openSections.relational && (
              <div className="p-3 space-y-1.5">
                ...
                <ParamRow label="Attractor Count" value={config.relational?.attractorCount ?? 3} onChange={(val) => onChange({ relational: { enabled: false, ...config.relational, attractorCount: Math.round(val) } })} min={1} max={10} hardMin={1} hardMax={10} step={1} decimals={0} />
                <ParamRow label="Gravity Pull" value={config.relational?.attractorGravity ?? 1.6} onChange={(val) => onChange({ relational: { enabled: false, ...config.relational, attractorGravity: val } })} min={-50.0} max={50.0} hardMin={-1000.0} hardMax={1000.0} step={0.1} />
                <ParamRow label="Orbit Speed" value={config.relational?.orbitSpeed ?? 1.2} onChange={(val) => onChange({ relational: { enabled: false, ...config.relational, orbitSpeed: val } })} min={-20.0} max={20.0} hardMin={-200.0} hardMax={200.0} step={0.05} />
                <ParamRow label="Orbit Radius" value={config.relational?.orbitRadius ?? 220} onChange={(val) => onChange({ relational: { enabled: false, ...config.relational, orbitRadius: val } })} min={0} max={2000} hardMin={0} hardMax={20000} step={5} decimals={0} unit="px" />
                <ParamRow label="Relational Spin" value={config.relational?.relationalSpin ?? 1.2} onChange={(val) => onChange({ relational: { enabled: false, ...config.relational, relationalSpin: val } })} min={-30.0} max={30.0} hardMin={-500.0} hardMax={500.0} step={0.1} />
                <ParamRow label="Chaos Factor" value={config.relational?.chaosFactor ?? 0.0} onChange={(val) => onChange({ relational: { enabled: false, ...config.relational, chaosFactor: val } })} min={0.0} max={30.0} hardMin={0.0} hardMax={500.0} step={0.1} />
                <ParamRow label="Wander Speed" value={config.relational?.wanderSpeed ?? 0.5} onChange={(val) => onChange({ relational: { enabled: false, ...config.relational, wanderSpeed: val } })} min={0.0} max={10.0} hardMin={-100.0} hardMax={100.0} step={0.05} />
