...
export interface NativeDomainReading {
  ...
  m1: {revision:string; coordinate:string; quadrature:number[]; opposite_quadrature:number[]; standing:unknown};
...
export function projectNativeSources(sources:any,identity:{event_ref:string;subject_ref:string;generation:string}):NativeDomainReading {
  ...
  return {
    ...
    m1:{revision:m1.config.revision,coordinate:m1.config.selected_coordinate,quadrature:[...m1.carrier.quadrature],opposite_quadrature:[...m1.carrier.opposite_quadrature],standing:m1.standing},
...
export type NativeBasisEdit = {kind:'harmonic-row';row12:number}|{kind:'transcription';rna:boolean}|{kind:'damping';mode_ref:string;per_second:number};
...
export function editNativeBasis(sources:any,edit:NativeBasisEdit):any {
  ...
  if(edit.kind==='harmonic-row'){
    requireValue(Number.isInteger(edit.row12)&&edit.row12>=0&&edit.row12<=11,'harmonic row must be 0–11');
    ...
    next.m1.row12=edit.row12;next.m1.revision=String(revision+1n);
