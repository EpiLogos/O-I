...
export function FileSurface({binding,forceSource}:{binding:SurfaceBinding;forceSource?:boolean}) {
  ...
  const body=useRef<HTMLTextAreaElement>(null);const scrollKey=`oi-cradle.file-scroll:${binding.id}`;
  ...
  useEffect(()=>{
    ...
    void readFile(transport,binding.location).then(value=>{
      ...
      requestAnimationFrame(()=>{try{if(body.current)body.current.scrollTop=Number(localStorage.getItem(scrollKey)??0);}catch{}});
  ...
  return <section className="native-file-surface" aria-label={`File ${binding.title}`} aria-busy={pending}>
    ...
    {reading&&draft&&<div className="source-editor-scroll"><div className="source-editor-body"><div className="source-gutter" aria-hidden="true">{Array.from({length:Math.max(1,draft.content.split("\n").length)},(_,i)=><span key={i}>{i+1}</span>)}</div><textarea ref={body} className="source-textarea" aria-label={`${writable?"Editing":"Reading"} ${binding.title}`} readOnly={!writable||pending} value={draft.content} onChange={event=>change(event.target.value)} spellCheck={false} onScroll={event=>{try{localStorage.setItem(scrollKey,String(event.currentTarget.scrollTop));}catch{}}}/></div></div>}
