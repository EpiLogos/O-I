...
export interface ContextCue {id:string;bindingId:string;sourceRef?:string;start:number;end:number;text:string}
...
export function contextCues(sourceRef?:string,bindingId?:string):ContextCue[]{return cues.filter(item=>sourceRef?item.sourceRef===sourceRef:item.bindingId===bindingId);}
