...
export function useMaterialContext(container:RefObject<HTMLDivElement>,binding:SurfaceBinding,lifecycleKey:string){
 useEffect(()=>{
  ...
  const present=(frame:HTMLIFrameElement,value:unknown)=>{if(disposed)return;const key=presentPageObservation(binding,value,async (key,documentId)=>!!await rpc(frame,'validate',{key,documentId}),undefined,(key,documentId,enabled)=>{void rpc(frame,'mark',{key,documentId,enabled});},frame.dataset.fileRevision,frame.dataset.workingCopy==="true");if(key){observations.add(key);if(observations.size>64){const oldest=observations.values().next().value!;observations.delete(oldest);releaseObservation(oldest);}}};
  ...
  return()=>{for(const key of observations)releaseObservation(key);observations.clear();disposed=true;clearInterval(timer);window.removeEventListener('message',receive);root.removeEventListener('load',loaded,true);root.removeEventListener('oi:page-attach-selection',attach);requests.forEach(request=>{clearTimeout(request.timer);request.resolve(null);});requests.clear();};
