...
export function useMaterialContext(container:RefObject<HTMLDivElement>,binding:SurfaceBinding,lifecycleKey:string){
 useEffect(()=>{
  ...
  const tick=async()=>{if(disposed||busy)return;const frame=root.querySelector<HTMLIFrameElement>('iframe[data-page-context]');if(!frame)return;
   ...
   busy=true;try{const scope=mode();if(frame!==lastFrame||scope!==lastMode){const accepted=await rpc(frame,'mode',{scope,ink:getComputedStyle(document.body).getPropertyValue('--oi-accent-ink').trim()});if(!accepted)return;lastFrame=frame;lastMode=scope;}if(scope==='components')present(frame,await rpc(frame,'take'));}finally{busy=false;}};
