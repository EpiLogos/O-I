...
export function ContextActions({bindings,accompanying}:{bindings:Record<string,SurfaceBinding>;accompanying:LayoutState["accompanying"]}){
 ...
 useEffect(()=>{let generation=0;const take=(event:Event)=>{const next=(event as CustomEvent<Candidate>).detail;if(!next?.text?.trim()||!next.bindingId)return;const current=++generation;const binding=latest.current.bindings[next.bindingId];if(!binding)return;
  ...
  };window.addEventListener("oi:context-options",take);return()=>{generation++;window.removeEventListener("oi:context-options",take);};},[]);
