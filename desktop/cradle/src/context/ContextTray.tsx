...
export function ContextTray({bindings,accompanying}:{bindings:Record<string,SurfaceBinding>;accompanying:LayoutState["accompanying"]}){
 ...
 useEffect(()=>{
  ...
  const validate=async(item:PreparedItem)=>{
   ...
   if(anchor.kind==="observation"){if(!await observationIsCurrent(anchor.key))throw new Error("The observed page/component changed or is closed. Review the captured snapshot in Context.");return;}
  ...
  const take=async(event:Event)=>{
   ...
   try{
    ...
    if(next.kind==="element"){if(!next.observationKey||!await observationIsCurrent(next.observationKey))throw new Error("The page selection changed. Select it again.");}
    else {
    ...
    const session=companion?.project===project?companion.ref:undefined;
    // Starts the existing owner service only, never a provider/Agent turn.
    await encounter(latest.current.kernel.transport,project,{action:"start"});
    const current=await nativeContext(latest.current.kernel.transport,project,session);
    const result=await nativeContext(latest.current.kernel.transport,project,session,{operation:"edit",basis:current.revision,mutation:{operation:"add",selection}});
  ...
  const changed=(event:Event)=>{const detail=(event as CustomEvent<{project:string;value:PreparedContext}>).detail;if(!detail?.value)return;const c=detail.value;
   const companion=latest.current.accompanying;
   if(companion&&c.scope.agent_session!==companion.ref)return;
