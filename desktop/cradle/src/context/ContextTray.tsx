...
import "./context.css";
type Candidate={bindingId:string;kind:string;text:string;sourceRef?:string;start?:number;end?:number;revision?:string;workingCopy?:boolean};
...
export function ContextTray({bindings,accompanying}:{bindings:Record<string,SurfaceBinding>;accompanying:LayoutState["accompanying"]}){
 ...
 const add=async()=>{if(!selected?.ref||!selected.project||!binding)return;setBusy(true);setError(undefined);
  try{
   ...
   else if(binding.kind==="source"&&binding.ref){const buffer=kernel.snapshot.buffers[binding.ref];if(!buffer||!revision||buffer.base_revision!==revision)throw new Error("The source changed since selection. Select the passage again.");canonical=buffer.content;working=buffer.dirty;}
   ...
   const metadata=[binding.title,origin,revision?`revision ${revision}`:"observed excerpt",working?"working copy — unsaved":""].filter(Boolean).join(" · ");
   const quoted=candidate.text.split("\n").map(line=>`> ${line}`).join("\n");
   const read=await encounter<EncounterReading>(kernel.transport,selected.project,{action:"view",agent_session:selected.ref});
   await encounter(kernel.transport,selected.project,{action:"draft",agent_session:selected.ref,basis:read.draft.revision,text:[read.draft.text,`@context — ${metadata}\n${quoted}`].filter(Boolean).join("\n\n")});
 ...
 return <div className="context-scrim" onMouseDown={e=>{if(e.target===e.currentTarget)close();}}><section ref={tray} className="context-tray" role="dialog" aria-modal="true" aria-label="Include selected context" tabIndex={-1} onKeyDown={keyboard}><header><h2>Include selected context</h2><button aria-label="Close context selection" onClick={close} disabled={busy}>×</button></header><p>{binding?.title??"Selection"} · {candidate.text.length} characters</p><pre>{candidate.text}</pre><p className="context-origin">{candidate.sourceRef??binding?.terminal?.cwd??"Observed material"}</p><label>Conversation <select aria-label="Context destination" value={target||choices[0]?.id||""} onChange={e=>setTarget(e.target.value)}>{choices.map(b=><option key={b.id} value={b.id}>{b.title} · {b.project}</option>)}</select></label>{!choices.length&&<p>Open an agent conversation to include this selection.</p>}<p>The excerpt and its provenance will be added to the shared draft. Use Send in that conversation when ready.</p>{error&&<p role="alert">{error}</p>}<button disabled={busy||!!error||!selected||!binding} onClick={()=>void add()}>{busy?"Adding…":"Add to draft"}</button></section></div>;
}
