...
export function useBrowserContext(binding:SurfaceBinding,native:boolean,mode:string,loading:boolean|undefined,control:(action:string,args?:Record<string,unknown>)=>Promise<unknown>){
 ...
 useEffect(()=>{
  ...
  const present=(reading:Result|null)=>{if(!reading||disposed)return;presentPageObservation(binding,reading.result,async (key,documentId)=>{const validation=await rpc('validate',{key,documentId});return validation?.url===reading.url&&validation.result===true;},reading.url,(key,documentId,enabled)=>{void rpc('mark',{key,documentId,enabled});});};
  void listen<Result>('oi:browser-picked',event=>{if(event.payload.id===binding.id)present(event.payload);}).then(fn=>{if(disposed)fn();else unpick=fn;});
  ...
  attach.current=()=>{void rpc('selection').then(present);};
