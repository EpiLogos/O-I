...
function PreparedRow({project,item,context,busy,onReveal,onRemove,onExpression}:{project?:string;item:PreparedItem;context:PreparedContext;busy:boolean;onReveal:()=>void;onRemove:()=>void;onExpression:(value:string)=>void}){
 ...
 return <li className="prepared-context-item" data-selection-id={item.id}>
  ...
  <details><summary>Selection details</summary><p className="oi-ref">{s.source_ref}</p><small>{s.source_revision??"observed page"} · {s.anchor.kind==="text"?`UTF-16 ${s.anchor.start}–${s.anchor.end}`:`${s.anchor.role} · ${s.anchor.selector}`}</small><pre>{s.text}</pre>
   ...
   <label>Vāk expression<input aria-label={`Vāk expression for ${s.title}`} value={expression} onChange={e=>setExpression(e.target.value)} spellCheck={false}/></label><button type="button" disabled={busy||expression===item.canonical_expression} onClick={()=>onExpression(expression)}>Save expression with AIKit</button><SelectionResolution project={project} expression={expression}/>
  </details>
 </li>;
}
