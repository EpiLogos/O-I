...
import {projectConstruction, saveCompositionFile, attachCompositionReturn, type ArtifactReturn} from './constructionProjection';
import './wikiConstruction.css';

export interface ConstructionCheckpoint {draft: ConstructionDraft; pending?: ConstructionRequest; artifact?: {location: import('../kernel/types').CentralLocation; revision: string; expression_ref: string}; saved?: boolean}
type Props = {binding: SurfaceBinding; open: boolean; incoming?: WikiPassage; checkpoint?: ConstructionCheckpoint; onCheckpoint: (value: ConstructionCheckpoint) => void; onClose: () => void; onNavigate: WikiNavigate; onSaved: () => void};
...
export function WikiConstructionPanel({binding, open, incoming, checkpoint, onCheckpoint, onClose, onNavigate, onSaved}: Props) {
  ...
  return <aside className="wiki-construction" hidden={!open} aria-label="Constellation authoring">
    ...
    <div ref={host} className="wiki-construction-stage" hidden={!document}/>
    ...
    {draft.basis?.[CONSTRUCTION].compositions?.length ? <section aria-label="Returned compositions"><h3>Returned work</h3>{draft.basis[CONSTRUCTION].compositions!.map(item=><p key={item.reference}><button className="oi-action" onClick={()=>summonExpression(item.reference)}>{item.kind} · r{item.revision}</button></p>)}</section> : null}
  </aside>;
}
