...
interface Props {nodes:GraphNode[];positions:Point[];model?:GraphReading;camera:Camera;selected?:string;focused:Set<string>;contextual?:Set<string>;labels?:'automatic'|'all'|'focus';arrows?:boolean;minZoom:number;maxZoom:number;onCamera:(camera:Camera)=>void;onOpen:(node:GraphNode)=>void;onClear:()=>void}
...
export const GraphCanvas=memo(function GraphCanvas(props:Props) {
  ...
  useEffect(()=>{engine.current?.target(props.camera);engine.current?.redraw();},[props.camera.x,props.camera.y,props.camera.zoom,props.nodes,props.positions,props.model,props.selected,props.focused,props.contextual,props.labels,props.arrows]);
