...
export type SharedFieldRequest=Record<string,unknown>&{kind:"status"|"snapshot"|"read"|"publish"|"projection"|"participant"|"contribute"|"admit"|"reject"|"withdraw"|"contact"|"watch"};
...
export type SharedFieldReading=
  ...
  |{schema:"oi.shared-field.reading/v1";ref:string;state:"hosted";target:SharedFieldTarget;field_ref:string|null;entry:HostedEntry;projections:HostedProjection[];relations:HostedRelation[];relation_errors?:HostedRelationError[];contributions:HostedContribution[];neighbourhood:unknown;my_authority:HostedAuthority[];my_watches:HostedWatch[];status:unknown}
