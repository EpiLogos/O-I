...
export interface HostedContribution {contribution_ref:string;field_ref:string;contributor_participant_ref:string;ingress_ref:string|null;contract:{schema:"oi.contribution/v1";contribution_ref:string;field_ref:string;contributor_participant_ref:string;created_at:string;mode:string;target:{kind:string;ref:string;revision?:string};relation:{kind:string};representation:{kind:string;payload?:unknown;ref?:string};provenance:unknown[];addressing?:unknown}}
export interface HostedPendingContribution {ingress_ref:string;field_ref:string;contribution_ref:string;contributor_participant_ref:string;received_at_micros:string;payload_fingerprint:string;contract:HostedContribution["contract"]}
export interface SharedFieldContributionResult {schema:"oi.shared-field.contribution-result/v1";contribution_ref:string|null;ingress_ref:string;field_ref:string|null;state:"quarantined"|"admitted"|"withdrawn"|"rejected"|string}
...
export interface SharedFieldSnapshot {schema:"oi.shared-field.snapshot/v1";target:SharedFieldTarget;transport_identity:string;status:{healthy?:boolean;transport?:{state?:string};[key:string]:unknown};fields:HostedField[];participants:HostedParticipant[];projections:HostedProjection[];entries:HostedEntry[];relations:HostedRelation[];relation_errors?:HostedRelationError[];contributions:HostedContribution[];owner_pending_contributions:HostedPendingContribution[];my_authority:HostedAuthority[];my_contribution_receipts:SharedFieldContributionResult[];my_watches:HostedWatch[];my_contacts:unknown[];counts:{fields:number;participants:number;projections:number;entries:number;relations:number};entry_fields:Record<string,string>;relation_fields:Record<string,string>}
...
export type SharedFieldReading=
  ...
  |{schema:"oi.shared-field.reading/v1";ref:string;state:"hosted";target:SharedFieldTarget;field_ref:string|null;entry:HostedEntry;projections:HostedProjection[];relations:HostedRelation[];relation_errors?:HostedRelationError[];contributions:HostedContribution[];owner_pending_contributions:HostedPendingContribution[];participants:HostedParticipant[];neighbourhood:unknown;my_authority:HostedAuthority[];my_watches:HostedWatch[];status:unknown}
