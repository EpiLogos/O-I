...
function binding(member:LocalMember):SubjectBinding{return {subject_ref:member.node.ref,native_owner:member.reading.provider??member.node.native_owner,presentation_role:"thing",sources:member.sources??(member.reading.evidence.length?member.reading.evidence.map(source):[source(member.reading.resource)]),readings:[rr(member)],actions:[]};}
...
export async function projectProvidedLocalWhole(transport:KernelTransportStatus,surface:string,title:string,whole:LocalWhole,signal?:AbortSignal):Promise<ProjectionOutcome>{
 try{
  ...
  try{result=await expression(transport,{operation:"inspect",expression_ref});}
   ...
   if(!(error instanceof Error)||error.message!=='Expression is not open')throw error;
   signal?.throwIfAborted();
