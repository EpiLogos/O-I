...
export interface LocalMember {node:GraphNode;reading:KnowledgeReading;sources?:ReadingRef[]}
...
function address(node:GraphNode):KnowledgeAddress|undefined{return node.kind==="wiki-node"||node.kind==="wiki-space"||node.kind==="flow"||node.kind==="knowledge-subject"?{kind:"wiki",value:node.ref}:node.kind==="file"?{kind:"source",value:node.ref}:undefined;}
...
export async function readLocalWhole(transport:KernelTransportStatus,project:string|undefined,locus:GraphNode,pins:GraphNode[]=[],participation?:OwnerParticipation,rootAddress?:KnowledgeAddress):Promise<LocalWhole>{
 ...
 const admitted=[locus.ref,...pins.map(n=>n.ref),...relations.nodes.map(n=>n.resource)].filter((ref,i,a)=>a.indexOf(ref)===i).slice(0,KNOWLEDGE_EXPRESSION_LIMIT),members:LocalMember[]=[],unaddressed:string[]=[];
 for(const ref of admitted){const node=graphNodes.get(ref),a=ref===locus.ref?root:node&&address(node);if(node&&!a){unaddressed.push(`${node.kind}: ${node.ref}`);continue;}if(node&&a){const reading=await knowledge<KnowledgeReading>(transport,project,{action:"read",address:a});members.push({node:{...node,native_owner:reading.provider,provenance:{source:reading.authority,revision:reading.revision,detail:reading.evidence}},reading});}}
 ...
 return {locus:locus.ref,members,edges,truncated:relations.truncated||relations.nodes.length>members.length,warnings:[...relations.warnings,...(unaddressed.length?[`Native relation members supplied no readable address: ${unaddressed.join(", ")}`]:[])],pinned:pins.map(n=>n.ref).filter(ref=>refs.has(ref)),grammar:validateParticipation(participation,members),relationBindingsUnavailable:edges.length};
}
...
function binding(member:LocalMember):SubjectBinding{return {subject_ref:member.node.ref,native_owner:member.reading.provider,presentation_role:"thing",sources:member.sources??(member.reading.evidence.length?member.reading.evidence.map(source):[source(member.reading.resource)]),readings:[rr(member)],actions:[]};}
...
export async function projectProvidedLocalWhole(transport:KernelTransportStatus,surface:string,title:string,whole:LocalWhole,signal?:AbortSignal):Promise<ProjectionOutcome>{
 try{
  ...
  result=await expression(transport,{operation:"edit",expression_ref,expected_revision:result.document.revision,actor:"human:knowledge-expression",changes});
