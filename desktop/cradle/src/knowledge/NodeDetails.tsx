...
export function HostedReadingBody({reading}:{reading:SharedFieldReading}) {
  ...
  return <div data-hosted-state="hosted" data-hosted-ref={entry.ref}>
    ...
    <details className="knowledge-provenance" open><summary>Projections · {projections.length}</summary><ul>{projections.map(p=><li key={`${p.projection_ref}@${p.projection_revision}`} data-projection-ref={p.projection_ref} data-projection-revision={p.projection_revision} data-source-revision={p.source.revision}><code>{p.projection_ref}</code> · projection revision {p.projection_revision} · source revision <code>{p.source.revision}</code> · {p.state} · published by {p.publisher_participant_ref}</li>)}</ul>{!projections.length&&<p>No Projection names this entry or its world.</p>}</details>
    <details className="knowledge-related"><summary>Hosted relations · {relations.length}</summary><ul>{relations.map((r,i)=><li key={i}><span>{r.from===entry.ref?r.to:r.from}</span><small>{r.relation} · {r.origin}</small></li>)}</ul></details>
    <details className="knowledge-raw"><summary>Neighbourhood, contributions ({contributions.length}) and the full hosted reading</summary><pre>{JSON.stringify({entry,projections,relations,contributions,neighbourhood},null,2)}</pre></details>
  </div>;
}
export function NodeDetails({node,reading,hosted,error,project,onClose,onPromote,onOpen,native,rect,extent,onGeometry,storageError,disclosures,related,onRelated,transport,onActionDispatched}:{disclosures:GraphNode[];related:{edge:GraphEdge;node?:GraphNode}[];onRelated:(node:GraphNode)=>void;node:GraphNode;reading?:KnowledgeReading;hosted?:SharedFieldReading;error?:string;project?:string;onClose:()=>void;onPromote:()=>void;onOpen:OpenKnowledge;native:boolean;rect:DetailRect;extent:Extent;onGeometry:(rect:DetailRect)=>void;storageError?:string;transport:KernelTransportStatus;onActionDispatched:()=>void}) {
  ...
  const noLocalAddress=`No local address: this subject lives in the hosted Shared Field${hostedTarget?` (${hostedTarget})`:""}, not in a native knowledge body on this device`;
  const promote=async(placement:"page"|"window")=>{const address=graphAddress(node);if(!address){setFailure(noLocalAddress);return;}setPending(true);setFailure(undefined);try{await onOpen(address,node.label,project,placement);onPromote();}catch(e){setFailure(String(e));}finally{setPending(false);}};
  return <section className="knowledge-detail-dialog" role="dialog" aria-modal="false" aria-label={`Details: ${node.label}`} style={{left:rect.x,top:rect.y,width:rect.width,height:rect.height}} onKeyDown={e=>{if(e.key==="Escape"){e.preventDefault();e.stopPropagation();onClose();}}}>
    ...
    <article className="knowledge-detail-body" aria-busy={!reading&&!hosted&&!error}>
      ...
      <details className="knowledge-provenance" open={isHosted}><summary>Provenance & identity</summary><dl><dt>Reference</dt><dd>{node.ref}</dd><dt>Source</dt><dd>{node.provenance.source}</dd>{isHosted?<>{hostedTarget&&<><dt>Hosted at</dt><dd data-hosted-target={hostedTarget}>{hostedTarget}</dd></>}{node.provenance.revision&&<><dt>Projection revision</dt><dd data-projection-revision={node.provenance.revision}>{node.provenance.revision}</dd></>}{node.provenance.detail?.[1]&&<><dt>Entry revision</dt><dd data-entry-revision={node.provenance.detail[1]}>{node.provenance.detail[1]}</dd></>}</>:node.provenance.revision&&<><dt>Revision</dt><dd>{node.provenance.revision}</dd></>}</dl>{!isHosted&&node.provenance.detail?.map((text,i)=><p key={i}>{text}</p>)}</details>
    ...
    <footer className="knowledge-detail-footer"><button disabled={pending||!reading||isHosted} title={isHosted?noLocalAddress:undefined} onClick={()=>void promote("page")}>Open in tab <span aria-hidden="true">↗</span></button><button disabled={pending||!reading||!native||isHosted} title={isHosted?noLocalAddress:native?"Open this same subject in a native window":"Native popout is available in the desktop app"} onClick={()=>void promote("window")}>Pop out <span aria-hidden="true">↗</span></button></footer>
