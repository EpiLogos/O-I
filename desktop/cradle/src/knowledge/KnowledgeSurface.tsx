...
export function KnowledgeSurface({binding,onOpen}: {binding:SurfaceBinding;onOpen:OpenKnowledge}) {
  ...
  useEffect(()=>{const request=binding.view?.constructionFrame;if(request){setConstructionFrame(request.ref);setConstructionOpen(true);}},[binding.view?.constructionFrame?.requestId]);
  ...
  const openTechne=useCallback(async(node:GraphNode)=>{
    ...
    try{await openSubjectInTechne(transport,binding.project,node,model?.formations??[]);setTechne(undefined);}
    catch(error){setTechne({ref:node.ref,pending:false,failure:error instanceof Error?error.message:String(error)});}
  },[transport,binding.project,model]);
