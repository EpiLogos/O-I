...
export function PageExpression({page,fileRevision,pageRef}:{page:PageDocument;fileRevision:string;pageRef:string}){
 ...
 useEffect(()=>{if(!currentDocument||presentation.current)return;try{const handle=stage.present({id:presentationId,plane:"overlay",recipe:"",config:expressionConfig(currentDocument),sceneRef:currentDocument.selection.scene_ref});if(!handle)throw new Error(stage.error??"The accepted Expression renderer is unavailable");presentation.current=handle;handle.setContainer(inline.current);setStageReady(true);}catch(cause){setStageReady(false);setError(String(cause));}return()=>{presentation.current?.release();presentation.current=null;setStageReady(false);};},[currentDocument,presentationId,stage]);
