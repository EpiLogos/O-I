...
export function validatePage(doc) {
  ...
  if (doc.page.expression !== undefined && doc.page.expression !== null) validateExpressionPresentation(doc.page.expression);
...
function renderExpressionFallback(expression,title) {
  const fallback=expression.representations.find(item=>item.representation.availability==="available");
  let body=`<p>The live Expression renderer is not available in this standalone page.</p>`;
  if(fallback?.kind==="image"&&allowedLink(fallback.href))body=`<img src="${escape(fallback.href)}" alt="${escape(title||"Expression capture")}" loading="lazy">`;
  else if(fallback?.kind==="video"&&allowedLink(fallback.href))body=`<video src="${escape(fallback.href)}" controls preload="metadata"></video>`;
  else if(fallback?.kind==="html"&&typeof fallback.html==="string")body=`<iframe title="Frozen Expression" sandbox="" srcdoc="${escape(fallback.html)}"></iframe>`;
  return `<section class="page-expression" data-expression-ref="${escape(expression.expression_ref)}" data-expression-revision="${expression.expression_revision}" aria-label="Expression"><div class="page-expression-body">${body}</div><p class="page-expression-state">${fallback?`${escape(fallback.kind)} fallback · ${escape(fallback.representation.ref)} · ${escape(fallback.representation.revision)}`:"Expression unavailable"}</p></section>`;
}
