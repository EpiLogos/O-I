 ...
 return <section className="project-returns" aria-label="Returns">
  ...
  {open&&<div className="return-detail">
    <dl>
      ...
      <dt>Proposed operation</dt><dd>{String((open.record.proposal as {operation?:string}).operation??open.record.proposal)}</dd>
    ...
    {!open.included&&open.record.status!=="included"&&<div className="return-actions">
      ...
      {(open.record.status==="including"||open.record.status==="uncertain")&&<p role="status">Inclusion is {open.record.status}; recovery runs through the owner (central.receiving.recover).</p>}
    </div>}
