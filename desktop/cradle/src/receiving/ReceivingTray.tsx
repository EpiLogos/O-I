...
/** The project's receiving field (Wave 6E): what arrived, from whom, against
 * which document — reviewed and included only through Central's native
 * revision-checked operations. Arrival never edits a document; this tray
 * neither mints identities nor widens disclosure. Owner ruling 7
 * (DESKTOP-LANGUAGE.md, 2026-09-22): nothing in the app says "returns" — the
 * surface renders the arrived material itself, named by the native field it
 * reads (receiving). When the bound Central owner does not expose receiving,
 * the tray is quietly absent — the desktop never advertises a capability its
 * owner cannot do. */
export function ReceivingTray({project,refresh}:{project:string;refresh:number}) {
 ...
 return <section className="project-receiving" aria-label="Receiving">
  <header><span>Receiving</span>{page&&<small aria-label="Receiving summary">{shown.length?`${shown.length} in the receiving field`:"receiving field is clear"}</small>}<button className="receiving-refresh" aria-label="Refresh receiving" disabled={pending} onClick={load}><Glyph name="refresh" size={12}/></button></header>
  {shown.map(row=><button key={row.return_ref} className={`receiving-row ${row.now_ref?"receiving-has-now":""}`} data-now-ref={row.now_ref??undefined} aria-expanded={open?.return_ref===row.return_ref} onClick={()=>void expand(row)}>
  ...
  {open&&<div className="receiving-detail">
    <dl>
      ...
      <dt>Proposed operation</dt><dd>{String((open.record.proposal as {operation?:string}).operation??open.record.proposal)}{anchor&&<span className="receiving-anchor"> — {anchor}</span>}</dd>
    ...
    {!open.included&&open.record.status!=="included"&&<div className="receiving-actions">
      ...
      {open.record.status==="accepted"&&<button className="receiving-accept" disabled={pending} onClick={include}>Include into the document</button>}
      {(open.record.status==="including"||open.record.status==="uncertain")&&<button className="receiving-recover" disabled={pending} onClick={recover}>Recover inclusion</button>}
    </div>}
