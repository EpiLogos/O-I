...
export function IconTab({label, icon, selected, mark, children, iconContent, className = '', choice = false, ...props}: ButtonHTMLAttributes<HTMLButtonElement> & {label: string; icon?: GlyphName; iconContent?: ReactNode; selected: boolean; mark?: 'dot' | 'attention'; choice?: boolean}) {
  return <button {...props} type="button" role={choice ? "radio" : "tab"} aria-selected={choice ? undefined : selected} aria-checked={choice ? selected : undefined} aria-label={label} title={props.title ?? `${label}${mark === 'attention' ? ' — needs you' : mark === 'dot' ? ' — new activity' : ''}`} tabIndex={props.tabIndex ?? (selected ? 0 : -1)} className={`icon-tab ${className}`}>
    {iconContent ?? (icon && <Glyph name={icon} size={14}/>)}{children}{mark && <span className="icon-tab-mark" data-mark={mark} aria-hidden="true">{mark === 'attention' ? '!' : ''}</span>}
  </button>;
}
...
export function IconTabStrip({items, current, onSelect, children, orientation = 'horizontal', className = '', choice = false, showLabels = false, crossAxisArrows = false, ...props}: Omit<HTMLAttributes<HTMLDivElement>, 'onSelect'> & {items?: readonly IconTabItem[]; current?: string; onSelect?: (id: string) => void; children?: ReactNode; orientation?: 'horizontal' | 'vertical'; choice?: boolean; showLabels?: boolean; crossAxisArrows?: boolean}) {
  return <div {...props} className={`icon-tab-strip oi-scroll ${className}`} role={choice ? "radiogroup" : "tablist"} aria-orientation={orientation} onKeyDown={event => {
    ...
    const target = event.key === 'Home' ? tabs[0] : event.key === 'End' ? tabs[tabs.length - 1] : (event.key === next || ((choice || crossAxisArrows) && event.key === 'ArrowDown')) ? tabs[(index + 1) % tabs.length] : (event.key === previous || ((choice || crossAxisArrows) && event.key === 'ArrowUp')) ? tabs[(index - 1 + tabs.length) % tabs.length] : undefined;
