...
export function modeExtraPlanes(mode: WorkspaceMode, subject: PanelSubject, accompanying?: PanelAccompanying, _onMessage?: (message: string) => void, _host?: FactoryPanelHost, _full?: boolean, opens?: TaPaneOpens, project?: string): {id: string; label: string; body: ReactNode}[] {
  ...
  if (mode === "factory") return [
    ...
    {id: "factory-context", label: "Context", body: <Suspense fallback={null}><div className="factory-context-stack"><ContextPaneMount opens={opens} dataPlane="factory-context" project={project ?? accompanying?.project} session={accompanying?.ref}/>{!opens?.sideTabs?.length && <FactoryContextSlice/>}</div></Suspense>},
  ];
