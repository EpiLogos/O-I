 ...
 * not settings; they stay in the developer record, not on the
...
import {axisWord, availabilityWord, humanizeKey} from "./vocabulary";
    ...
    if (!secret && isBulky(current?.value)) {
      return (
        <details className="native-setting">
          ...
          {setting.native_path && <p className="native-setting-path">Set through: {setting.native_path}</p>}
        </details>
      );
    }
    return (
  ...
  return (
    <details className="native-setting">
      ...
      <dl className="native-axes">
        {compared.map(name => (
          <div className="native-axis-row" key={name}><dt>{axisWord(name)}</dt><dd>{axisValue(axisByName[name], secret)}</dd></div>
        ))}
        {staged_ && (
          <div className="native-axis-row">
            <dt>Staged by the product{staged_.stage_state !== "prepared" ? ` (${humanizeKey(staged_.stage_state)})` : ""}</dt>
      ...
      <summary>
        ...
        <span className="native-owner-meta">
          ...
          <span className={`native-owner-availability is-${availability.state}`}>
            {availabilityWord(availability.state)}
