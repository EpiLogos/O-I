...
export function EncounterList({project,onOpen,activeRef}:{project:string;onOpen:(row:EncounterRow)=>Promise<void>;activeRef?:string}) {
 ...
 }).catch(error=>{if(live)setError(String(error));}).finally(()=>{if(live)setPending(false);});return()=>{live=false;};},[project,refresh,kernel.transport]);
 return <div className="project-encounters" aria-busy={pending}><button className="project-availability" disabled={pending} onClick={()=>setRefresh(value=>value+1)}>Refresh conversations</button>{error?<p className="project-availability" role="status">{error}</p>:!rows?<Loading label="Reading chats and tasks…" scope="surface"/>:<>
