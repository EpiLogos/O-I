...
import {AddressedRecipientPicker,type AddressableParticipant,type AddressableParticipantsState} from "./AddressedRecipientPicker";
...
export function AddressedComposer({disabled,dispatch,history,service,agentSession,task,group,addressable,onGroupSend,onDispatchFields,onSend}:{disabled:boolean;dispatch:DispatchState;history:DeliveryHistoryEntry[];service?:{running:boolean;pid?:number;detail?:string};agentSession?:string;task?:EncounterTaskReading;group?:GroupState;addressable?:AddressableParticipantsState;onGroupSend?:(sender:string,recipients:GroupRecipient[],packet:AddressedPacket)=>void;onDispatchFields?:(fields:AddressedFields)=>void;onSend:(turn:AddressedTurn,fields:AddressedFields)=>void}) {
  ...
  const [selectedRecipients,setSelectedRecipients]=useState<AddressableParticipant[]>([]);
  const addressedParticipants=addressable??{kind:"awaiting-sender" as const};
  // A changed owner reading invalidates a stale selection before it can form a
  // send-group request. The sender/source filter is advisory; owner preflight
  // remains the effect boundary.
  useEffect(()=>{if(addressable?.kind!=="ready"){setSelectedRecipients([]);return;}
    setSelectedRecipients(previous=>{const current=previous.filter(selected=>addressable.participants.some(participant=>participant.agent_session===selected.agent_session&&participant.agent_ref===selected.agent_ref&&participant.expected_binding_revision===selected.expected_binding_revision));return current.length===previous.length?previous:current;});
  },[addressable]);
  const groupReady=()=>!running&&!disabled&&addressedParticipants.kind==="ready"&&!!sender.trim()&&!!text.trim()&&selectedRecipients.length>0&&selectedRecipients.every(selected=>addressedParticipants.participants.some(candidate=>candidate.agent_session===selected.agent_session&&candidate.agent_ref===selected.agent_ref&&candidate.expected_binding_revision===selected.expected_binding_revision));
  const submitGroup=()=>{if(groupReady()&&onGroupSend)onGroupSend(sender.trim(),selectedRecipients.map(participant=>({agent_session:participant.agent_session,expected_binding_revision:participant.expected_binding_revision})),{text,source_refs:sourceRefs.split(/[\s,]+/).filter(Boolean),audience:selectedRecipients.map(participant=>participant.agent_ref)});};
  ...
  return <section className="encounter-addressed" aria-label="Addressed request">
    ...
    {addressable&&<details className="encounter-addressed-group">
      <summary>Addressed group</summary>
      <p>Choose from participants the owner has currently admitted for this sender and these shared source refs. Each selected participant receives an independently recorded delivery; this fanout does not create a shared conversation or membership.</p>
      <AddressedRecipientPicker disabled={disabled||running} participants={addressedParticipants} selected={selectedRecipients} onChange={setSelectedRecipients}/>
      <div className="encounter-addressed-actions">
        <span className="encounter-addressed-group-preview">{selectedRecipients.length||"no"} recipient{selectedRecipients.length===1?"":"s"} selected</span>
        <button className="encounter-addressed-group-send" disabled={!groupReady()||!onGroupSend} onClick={submitGroup}>Dispatch to selected participants</button>
      </div>
