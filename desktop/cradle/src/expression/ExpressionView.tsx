...
export function ExpressionView({initialExpressionRef}:{initialExpressionRef?:string}={}){
 ...
 return <section className="expression-editor" aria-label="Expression composition" data-expression-ref={document?.expression_ref} data-presenting={presenting}>
  ...
  <div className="expression-tools expression-identity">
   ...
   <label className="oi-field">Title<input className="oi-input" aria-label="Expression title" value={title} onChange={e=>setTitle(e.target.value)}/></label>
...
function RevisionInput({label,value,revision,numeric=false,disabled=false,onCommit}:{label:string;value:string|number;revision:number;numeric?:boolean;disabled?:boolean;onCommit:(value:string|number,basis:number)=>Promise<boolean>}){
 ...
 return <span><input aria-label={label} type={numeric?"number":"text"} step={numeric?"any":undefined} disabled={disabled} value={draft}
  ...
  {uncommitted&&<><button className="oi-action" onClick={()=>{const next=numeric?Number(draft):draft;void onCommit(next,revision).then(accepted=>{if(accepted)dirty.current=false;setUncommitted(!accepted);});}}>Apply {label} to revision {revision}</button><button className="oi-action" onClick={()=>{dirty.current=false;basis.current=revision;setDraft(String(value));setUncommitted(false);}}>Use current {label}</button></>}
 </span>;
}
