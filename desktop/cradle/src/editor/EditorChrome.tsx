...
export function EditorFrame({children,className="",toolbar,footer,label,data}:{children:ReactNode;className?:string;toolbar:ReactNode;footer:ReactNode;label:string;data?:Record<string,string|boolean|undefined>}) {
  ...
  return <section ref={root} className={`editor-frame ${className}`} data-editor-focused={focused} data-toolbar-collapsed={collapsed} aria-label={label} {...dataAttributes}>
    <header className="editor-command-line">
      ...
      {focused&&<button type="button" className="editor-collapse" aria-expanded={!collapsed} title={collapsed?"Show editing tools":"Hide editing tools"} onClick={()=>setCollapsed(value=>!value)}>{collapsed?"⌄":"⌃"}</button>}
...
export function useTextContextMenu(binding:SurfaceBinding,textarea:React.RefObject<EditorHandle>,writable:boolean,onChange?:(value:string)=>void,onAttach?:()=>void){
  ...
  const onContextMenu=(event:React.MouseEvent<HTMLDivElement>)=>{const el=textarea.current;if(!el)return;if(el.selectionStart===el.selectionEnd)return;event.preventDefault();event.stopPropagation();setMenu({x:event.clientX,y:event.clientY,start:el.selectionStart,end:el.selectionEnd});};
  const menuNode=menu?<div className="editor-text-menu" role="menu" style={{left:menu.x,top:menu.y}} onPointerDown={event=>event.stopPropagation()}>
    ...
    <button role="menuitem" onClick={()=>{textarea.current?.command("highlight-yellow");setMenu(null);}}>Highlight selection</button>
    <button role="menuitem" onClick={()=>{textarea.current?.command("highlight-clear");setMenu(null);}}>Remove highlight</button>
