...
export function EditorFrame({children,className="",toolbar,footer,label,data,presentationTools,interactive=true}:{children:ReactNode;className?:string;toolbar:ReactNode;presentationTools?:ReactNode;footer:ReactNode;label:string;interactive?:boolean;data?:Record<string,string|boolean|undefined>}) {
  ...
  return <EditorMode.Provider value={mode}><section ref={root} className={`editor-frame ${className}`} data-editor-focused={focused} data-toolbar-collapsed={collapsed} data-editor-mode={interactive?mode:"reading"} data-context-scope={interactive&&mode==="context"?"components":"off"} aria-label={label} {...dataAttributes}>
    <header className="editor-command-line"><div className="editor-command-tools" role={interactive?"toolbar":"group"} aria-label={interactive?`${label} modes`:`${label} presentation`}>
      {interactive&&<><button aria-label="Writing mode" aria-pressed={mode==="writing"} onClick={()=>{setMode("writing");setCollapsed(false);setHover(null);}}><Glyph name="edit" size={13}/></button>
    ...
    {interactive&&focused&&<button type="button" className="editor-collapse" aria-label={collapsed?"Show editing tools":"Hide editing tools"} aria-expanded={!collapsed} title={collapsed?"Show editing tools":"Hide editing tools"} onClick={()=>setCollapsed(value=>!value)}>{collapsed?"⌄":"⌃"}</button>}
    <div className="editor-content" onScrollCapture={()=>setHover(null)} onPointerMove={pick} onPointerLeave={()=>setHover(null)} onClickCapture={event=>{if(mode!=="context"||!hover||toolRegion(event.target as Element))return;event.preventDefault();event.stopPropagation();const id=root.current?.closest<HTMLElement>('[data-binding-id]')?.dataset.bindingId;if(id)window.dispatchEvent(new CustomEvent('oi:context-candidate',{detail:observeComponent(hover.node,id)}));setHover(null);}} onContextMenuCapture={event=>{if(!interactive||mode!=="writing")return;event.preventDefault();event.stopPropagation();setMenu({x:event.clientX,y:event.clientY});}}>{children}</div>
