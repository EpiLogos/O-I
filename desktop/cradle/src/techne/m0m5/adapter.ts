/**
 * PORT (T3, 2026-09-19): ported into the O:I cradle from QL-MEF adapters/techne/surfaces/cradle/src/techne (main @ dbff5cc == techne/t9-integrated-field, byte-identical there); import specifiers re-pointed to the cradle seam. No semantic change.
...
export function registerTechneAdapter(sourceRef: string, adapter: TechneAdapter): () => void {
  if (adapterOverrides.has(sourceRef)) throw new Error(`A Technē adapter override for ${sourceRef} is already registered`);
  adapterOverrides.set(sourceRef, adapter);
  return () => {
    if (adapterOverrides.get(sourceRef) === adapter) adapterOverrides.delete(sourceRef);
  };
}

/** Compose an adapter over one registered source. */
export function adapterForSource(source: TechneSource): TechneAdapter {
  const override = adapterOverrides.get(source.ref);
