...
export function AdoptionEntry({onApplied}: {onApplied: () => void}) {
  ...
  const [configuration, setConfiguration] = useState<ConfigPlaneSource>();
  ...
  return <section aria-label="Installation and capability setup" data-adoption-entry>
    ...
    <button type="button" disabled={transport.kind === "unavailable"} onClick={() => setOpen(true)}>Install and set up…</button>
    {transport.kind === "unavailable" && <p>Native installation is unavailable in this browser. Use the installed O:I terminal setup on the receiving machine; public Library reading needs no installer.</p>}
    {error && <p role="alert">{error}</p>}
    {open && !configuration && <AdoptionFlow controller={controller} onApplied={onApplied} onClose={() => setOpen(false)} onConfigure={() => void configure()}
      chooseGround={transport.kind === "tauri" ? async () => {
        const {invoke} = await import("@tauri-apps/api/core");
        return invoke<string | null>("choose_central_folder");
      } : undefined}/>}
    {open && configuration && <PlanDrawer source={configuration} requests={[]} settings={{}} startAtReview={false}
      onClose={() => setConfiguration(undefined)} onApplied={onApplied}/>}
  </section>;
}
