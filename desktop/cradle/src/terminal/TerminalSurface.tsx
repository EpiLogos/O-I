...
export function TerminalSurface({binding}:{binding:SurfaceBinding}){
 ...
 useEffect(()=>{
  ...
  const colours=()=>{const style=getComputedStyle(host.current!);return {background:style.backgroundColor,foreground:style.color,cursor:style.color};};
  let appliedColours=colours();
  const term=new Terminal({fontSize:12,fontFamily:'"SFMono-Regular", Menlo, monospace',cursorBlink:true,scrollback:5000,screenReaderMode:true,disableStdin:true,theme:appliedColours});
  ...
  const themeObserver=new MutationObserver(()=>{if(disposed)return;const next=colours();if(next.background!==appliedColours.background||next.foreground!==appliedColours.foreground){appliedColours=next;term.options.theme=next;}});
  for(let ancestor:HTMLElement|null=host.current;ancestor;ancestor=ancestor.parentElement)themeObserver.observe(ancestor,{attributes:true,attributeFilter:["class","style","data-theme"]});
  themeObserver.observe(document.head,{subtree:true,childList:true,characterData:true});
  ...
  const attachment=invoke<{lease:number;seq:number;snapshot:string;cwd:string}>("terminal_attach",{id:binding.id,cwd:binding.terminal?.cwd??null,attachment:binding.terminal?.attachment??null,dimensions:{cols:term.cols,rows:term.rows}}).then(async attached=>{
