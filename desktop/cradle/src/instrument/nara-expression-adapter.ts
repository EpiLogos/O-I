...
function validateSession(snapshot:FocusedInstrumentSnapshot,session:NaraExpressionSession){
  ...
  if(!present(session.earth_body.locus_ref)||loci.has(session.earth_body.locus_ref)||!present(session.earth_body.source.source_ref)||!present(session.earth_body.frame_ref))throw new Error("EarthBody must be a distinct source-qualified locus");
  ...
  if(session.portable.centre_locus_refs.length!==7||session.portable.earth_body_locus_ref!==session.earth_body.locus_ref)throw new Error("Portable Nara cues lost a presentation locus");
}
...
export function naraExpressionConfig(session:NaraExpressionSession):NativeConfig{
  ...
  const earth=entity("EarthBody","⊕",{x:0,y:-0.9,z:0});
...
function sourceTint(session:NaraExpressionSession):{tint:string;weight:number}{
  ...
  const hex=rgba.slice(0,3).map(value=>Math.round(Math.max(0,Math.min(1,value as number))*255).toString(16).padStart(2,"0")).join("");
  return {tint:`#${hex}`,weight:colour?.standing?0.72:0};
}
...
function presentationEntity(id:string,ordinal:number,resonance:number,session:NaraExpressionSession){
  ...
  return {id,x:0,y:-0.66+ordinal*0.22,z:0,scale:Math.max(0.72,Math.min(1.34,0.9+Math.abs(resonance)*0.22)),tint:colour.tint,tintWeight:colour.weight};
}
