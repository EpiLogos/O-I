...
export type FlowRequest={action:"flow_inspect";project:string;flow_ref:string}|{action:"flow_create";project:string;actor:string;actor_kind:"human";path:string;title:string}|{action:"flow_read";project:string;flow_ref:string}|{action:"flow_write";project:string;flow_ref:string;expected_revision:string;content:string;actor:string;actor_kind:"human"};
...
export async function inspectFlow(transport:KernelTransportStatus,project:string,flow_ref:string):Promise<FlowInspection>{
