...
/** The project's retained Flows, carried verbatim from Central's own listing.
 * Sorting/grouping is presentation; identity is the owner's FlowRef. */
export async function listFlows(transport:KernelTransportStatus,project:string|null):Promise<FlowRecord[]>{
 const result=await kernelOp(transport,{op:"flow",request:{action:"flow_list",project}});
 if(result.error||result.outcome?.result!=="flow")throw new Error(result.error??"Central did not return the Flow list");
 const response=result.outcome.response as {kind:string;listing?:{flows?:FlowRecord[]};error?:{message?:string;detail?:string}};
 if(response.kind==="failure")throw new Error(response.error?.message??response.error?.detail??"Central refused the Flow list");
 if(response.kind!=="flow_list"||!response.listing?.flows)throw new Error("Central returned no Flow list");
 return response.listing.flows;
}
