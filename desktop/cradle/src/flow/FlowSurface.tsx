...
export function FlowSurface({binding}:{binding:SurfaceBinding}){
 ...
 return <EditorFrame className="flow-surface" label="Flow editor"
  ...
  footer={<><span className="editor-path" title={`${binding.ref}\n${binding.flow?.flowRef}`}>{binding.project} / {binding.flow?.path}</span><span role="status">{status}</span><button title={capabilities?.write.reason??undefined} disabled={!loaded||busy||!revision||!writable||text===saved} onClick={()=>void save()}>Save · ⌘S</button></>}
 >
