...
export function FreshSurface({binding}:{binding:SurfaceBinding}){
 ...
 return <section className="fresh-surface" aria-label="New tab"><div>
  ...
  <details className="fresh-more"><summary>Project and document forms</summary>
   ...
   <nav className="fresh-docforms" aria-label="Open a document form">{DOCUMENT_FORMS.map(form=><button key={form.kind} disabled={busy} title={form.hint} onClick={()=>choose(form.kind)}><b>{form.label}</b><span>{form.hint}</span></button>)}</nav>
  </details>
