...
test('native temporal metadata reaches the active Scene Timeline with exact dates, precision and source identity',{skip:process.env.OI_NATIVE_AUTHORED_SCENE!=='1',timeout:120000},async()=>{
 for(const key of ['OI_KERNEL_BIN','OI_BIN','OI_AIKIT_BIN','OI_CENTRAL_CTRL_BIN'])assert.ok(process.env[key],key+' required');
 const retained=process.env.OI_NATIVE_TEMPORAL_EVIDENCE_DIR;
 const scratch=retained??await mkdtemp(join(tmpdir(),'oi-scene-temporal-')),root=join(scratch,'Central'),home=join(scratch,'oi-home');let child,stderr='';
 ...
 try{
  ...
  assert.ok(resolved.provenance.every(row=>row.source_ref===sourceRow.ref&&row.source_revision===String(sourceRow.revision)));
  ...
  const emptyInstruments=await api.readingInstruments(undated);assert.equal((await emptyInstruments.timeline.getTimelineWalk(emptyScene,{startYear:2019,endYear:2022})).earthboundNodes.length,0);
  if(retained)await writeFile(join(scratch,'native-temporal-proof.json'),JSON.stringify({schema:'oi.native-temporal-proof/v1',environment:{OI_CENTRAL_ROOT:root,CENTRAL_ROOT:root,OI_HOME:home,AIKIT_HOME:env.AIKIT_HOME,OI_BIN:env.OI_BIN,OI_AIKIT_BIN:env.OI_AIKIT_BIN,OI_CENTRAL_CTRL_BIN:env.OI_CENTRAL_CTRL_BIN},expression_ref:document.expression_ref,scene_ref:scene.scene_ref,revision:emptyDocument.revision,source_path:sourcePath,reading:resolved,timeline:walk,document:emptyDocument},null,2)+'\n');
