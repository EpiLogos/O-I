...
try{
 ...
 console.log(JSON.stringify({result:'passed',checks:['actual relation commit and independent readback','retained draft/scene/entity identities','durable lost-reply recovery without replay','source-owned relation refusal','tampered checkpoint refusal','concurrent revision conflict retains exact intent','deleting focused relation clears native selection'],native_calls:calls}));
}finally{
 if(child){const exited=child.exitCode===null?once(child,'exit'):null;try{process.kill(-child.pid,'SIGTERM');}catch{}if(exited)await exited;}
