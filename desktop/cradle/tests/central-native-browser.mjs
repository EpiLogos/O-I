...
function native(){
 const result=JSON.parse(execFileSync(bindings.ctrl,['--json','--root',bindings.root,'action','run','central.document.read','-'],{input:JSON.stringify({source_ref:bindings.source_ref,document_id:bindings.document_id}),encoding:'utf8',timeout:30000}));
...
try{
 for(const name of (process.env.CENTRAL_BROWSER_ENGINES??'chromium').split(',')){
  ...
  try{
   ...
   assert.equal(await input.inputValue(),text);check(name+': back and return retain unsaved original form');
   ...
   await input.waitFor();assert.equal(await input.inputValue(),text);check(name+': fresh renderer reopens native content');
