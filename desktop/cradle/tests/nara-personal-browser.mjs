...
server.middlewares.use('/personal',async(_req,res)=>{res.setHeader('content-type','text/html');res.end(await server.transformIndexHtml('/personal','<!doctype html><html><head><meta charset="utf-8"></head><body style="margin:20px;font-family:var(--oi-font-sans);color:var(--oi-foreground);background:var(--oi-surface)"><main id="root" style="max-width:640px"></main><iframe id="identity-app" src="/identity-app" title="Existing Expressions" style="width:940px;height:650px"></iframe><script type="module" src="/tests/nara-personal-page.tsx"></script></body></html>'));});
...
try{
 ...
 await page.getByRole('button',{name:'Centres',exact:true}).click();await page.getByRole('heading',{name:'Seven receiving centres',exact:true}).waitFor();const centre=page.getByRole('combobox',{name:'Centre',exact:true});await centre.waitFor();check(await centre.locator('option').count()===7,'seven independent receiving centres are not replaced by cymatic stations');await centre.selectOption('6');check(await centre.inputValue()==='6','the seventh receiving centre is independently selectable by its native ordinal');await page.getByRole('button',{name:'Review feedback source',exact:true}).click();await page.getByRole('button',{name:'Apply this reviewed change',exact:true}).click();await page.waitForFunction(()=>!document.querySelector('[aria-label="Review personal change"]'));check(owner.current.domain.embodied.centres[6].feedback_refs?.[0]===owner.current.domain.identity.slots[1].protected_value_ref.ref_id,'reviewed feedback is applied to the exact selected seventh centre and source');
 ...
 await page.locator('#identity-app').scrollIntoViewIfNeeded();await mkdir(artifacts,{recursive:true});await frame.locator('#stage').screenshot({path:fileURLToPath(new URL('controlled-private-native.png',artifacts))});
 await page.locator('#identity-app').scrollIntoViewIfNeeded();await frame.getByRole('button',{name:'Return to my Expression',exact:true}).click();await frame.waitForFunction(()=>!document.body.hasAttribute('data-private-identity'));check(await frame.evaluate(()=>JSON.stringify(window.__FIELD_STUDIES__.getDocument()))===ordinary,'return restores the same authored Expression, not a claimed GPU rewind');
