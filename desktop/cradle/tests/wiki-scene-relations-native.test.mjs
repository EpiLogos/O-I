...
import {readWikiRegister} from '../src/techne/wikiExpression.ts';
...
test('actual native Scene relations reach dated-source focus and filter rendered native endpoints',{skip:!artifact,timeout:120000},async()=>{
 ...
 try{
  ...
  const compiled=await build({stdin:{contents:"export {bundleFromTechneReadings} from '../../vendor/research-canvas/packages/desktop-api/src/techneBundle.ts';export {readingInstruments,nativeInstrumentTitles,nativeInstrumentNodeTags,nativeInstrumentSourceRelations} from './researchInstrumentsData.ts';export {kernelDocumentToJourney} from './kernelDocumentBridge.ts';export {TimelineRelationField} from '../../vendor/research-canvas/packages/canvas/src/timeline/TimelineRelationField.tsx';export {TimelineWalk} from '../../vendor/research-canvas/packages/canvas/src/timeline/TimelineWalk.tsx';export {TimelineWorkingSet} from '../../vendor/research-canvas/packages/canvas/src/timeline/TimelineWorkingSet.tsx';export {createTimelineStore} from '../../vendor/research-canvas/packages/canvas/src/timeline/timelineStore.ts';export {assembleTimelineWalk} from '../../vendor/research-canvas/packages/canvas/src/timeline/walk.ts';export {createElement,Children} from 'react';export {renderToStaticMarkup} from 'react-dom/server';",resolveDir:source},tsconfig:join(source,'../tsconfig.json'),bundle:true,platform:'node',format:'cjs',write:false,logLevel:'silent'});
  ...
  for(const relation of relations){const native=objects.find(row=>row.object==='edge'&&row.ref===relation.relation_ref),meta=native['aikit.constellation-relation/v1'];assert.equal(relation.standing,meta.standing);assert.deepEqual(relation.evidence,meta.evidence);assert.deepEqual(relation.provenance,native.provenance);assert.deepEqual(relation.evidence_refs,[...new Set(meta.evidence.map(row=>row.source_ref))]);}
  const registerReading=await readWikiRegister(transport,{key:'central',title:'Central'});assert.equal(registerReading.state,'ready');assert.equal(registerReading.relations.state,'available');
  const ql=wikiReadingPayload({register:{key:'central',title:'Central'},subject:{ref:'wiki:central',kind:'wiki-register',title:'Central'},reading:registerReading,document}),projected=api.bundleFromTechneReadings([ql]);
  let attributed=0;
  for(const relation of ql.whole.relations){const native=objects.find(row=>row.object==='edge'&&row.ref===relation.relation_ref);if(native?.['aikit.constellation-relation/v1']){const meta=native['aikit.constellation-relation/v1'];assert.equal(relation.standing,meta.standing);assert.deepEqual(relation.evidence_refs,[...new Set(meta.evidence.map(row=>row.source_ref))]);const projectedRelation=projected.relationships.find(row=>row.properties.native_relation_ref===native.ref);assert.ok(projectedRelation);assert.equal(projectedRelation.properties.standing,meta.standing);assert.deepEqual(projectedRelation.properties.evidence_refs,relation.evidence_refs);attributed++;}else{assert.equal(relation.standing,null,'provider authority is never invented as epistemic standing');}}
  assert.ok(attributed>0,'actual owner relations cross the register decoder and ql compatibility path');
  ...
  for(const edge of field.relationships){const native=relations.find(row=>row.binding_ref===edge.id);assert.equal(edge.properties.native_relation_ref,native.relation_ref);assert.equal(edge.properties.native_relation_revision,native.relation_revision);assert.equal(edge.relType,native.relation);assert.equal(edge.properties.standing,native.standing);assert.deepEqual(edge.properties.evidence,native.evidence);assert.deepEqual(edge.properties.evidence_refs,native.evidence_refs);assert.deepEqual(edge.properties.provenance,native.provenance);}
