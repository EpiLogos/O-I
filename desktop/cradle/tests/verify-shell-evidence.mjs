...
if(process.argv[1] && resolve(process.argv[1])===fileURLToPath(import.meta.url)) {
  ...
  const driverHash=createHash('sha256').update(readFileSync(new URL('./shell-recovery.browser.mjs',import.meta.url))).digest('hex');
  const results=[];
  for(const [name,count] of [['shell-recovery',12],['shell-setup',36],['visuals-preferences',22]]) {
    const bytes=readFileSync(new URL(`./artifacts/${name}/receipt.json`,import.meta.url));
    validateReceipt(JSON.parse(bytes),count,name==='shell-recovery'?{sourceRevision:sha,driverSha256:driverHash,runId:process.env.GITHUB_RUN_ID??null}:undefined);
