...
await runPlane.getByRole("button",{name:"Multi-lane running"}).click();
await page.waitForTimeout(400);
...
check(/Fixture — not native data/.test(await runPlane.locator('.factory-side-scenarios').innerText().catch(()=>"" )),"the fixture label is visible (not native data)");
...
check(/new since you started reading/.test(await runPlane.locator('.factory-side-traj-strip').innerText().catch(()=>"" )),"an arrival lands in a held view with a visible count");
await runPlane.getByRole("button",{name:"Resume live"}).click();
await page.waitForTimeout(200);
check(/Following the newest row/.test(await runPlane.locator('.factory-side-traj-strip').innerText().catch(()=>"" )),"resume live clears the held count");
...
await runPlane.getByRole("button",{name:"Blocked → resolved"}).click();
await page.waitForTimeout(400);
...
await agentsPlane.getByRole("button",{name:"Configured team"}).click();
await page.waitForTimeout(400);
...
await contextPlane.getByRole("button",{name:"Produced → review"}).click();
await page.waitForTimeout(400);
