...
export default async function run({page,baseUrl,check,metric,shot,channel,provision:p}){
  ...
  const labels=await overlay.locator('li strong').allTextContents();check(JSON.stringify(labels.slice(0,expected.length))===JSON.stringify(expected.map(hit=>hit.label)),'Search preserves native owner order and labels',{uiLabels:labels,expectedLabels:expected.map(hit=>hit.label)});
  ...
  // The canonical engine's glyph projection (7306b7b intake) renders the
  // projected whole at a measured aspect of ~1.17; the band still catches
  // the truncation/clipping defects, which drive the aspect far below one.
  check(glyphAspect>.85&&glyphAspect<1.35,'The native Wiki circle retains its whole shape at the desktop particle budget',{glyphAspect});await page.emulateMedia({colorScheme:'dark',reducedMotion:'reduce'});await page.waitForFunction(()=>document.body.dataset.theme==='dark');
  ...
  // The scene's palette re-grounding settles on the engine's own rendered
  // clock (software rendering is slow), so the field ground is waited for,
  // never sampled after a fixed nap.
  await page.waitForFunction(()=>{const canvas=document.querySelector('.knowledge-expression-host canvas');const host=document.querySelector('.knowledge-expression-host');return canvas&&getComputedStyle(canvas).backgroundColor===getComputedStyle(host).backgroundColor;},null,{timeout:20000});
