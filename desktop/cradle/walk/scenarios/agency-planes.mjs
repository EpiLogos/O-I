...
export default async function run({page,baseUrl,check,shot,channel,provision:p}) {
  ...
  const consent=page.locator(".encounter-composer .chat-permission");
  await consent.waitFor({timeout:30000});
  check((await consent.innerText()).includes("Fixture write"),"The provider's consent request is disclosed in words before any answer");
  ...
  await toConversation();
  if(await consent.locator(".chat-permission-scope").count())await consent.locator(".chat-permission-scope",{hasText:"This time"}).click();
