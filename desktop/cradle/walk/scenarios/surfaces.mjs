...
export default async function run({page,baseUrl,check,metric,shot,channel,provision:p}) {
  ...
  // The close button is the .tab's sibling inside .tab-entry, not its child.
  await page.locator('.tab-entry').filter({hasText:title(sources[4])}).first().locator('.tab-close').click();
  ...
  // The binding menu leads with focus-tab, names a move-to item per other
  // pane, then the operative frame actions in their fixed order.
  const actions=(await page.locator('.ctx-item').evaluateAll(es=>es.map(e=>e.dataset.actionRef))).filter(a=>!a.startsWith('surface.move-to'));
  check(JSON.stringify(actions)===JSON.stringify(['surface.focus-tab','surface.close','surface.maximize','surface.split-right','surface.split-down','surface.pin']),'Source frame menu discloses exactly its operative frame actions');
  ...
  check(await tab(sources[1]).count()===0,'Menu activation closes the selected view');
  await page.keyboard.press('Meta+Shift+t');
