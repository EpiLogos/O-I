 ...
 * fixture `oi` world, where the AIKit disclosure is honestly absent while
 * the aikit-driven reads stay real).
 *
 * The expectation is always computed from the LIVE owner reads the walk
 * runs itself — `aikit system --json` (the owner's own
 ...
 * pure contract as src/configuration/systemDisclosure.ts). */
export function liveDisclosureFacts() {
  const descriptor = readLiveSystemDisclosure();
  const section = (id) => descriptor.sections.find((row) => row.id === id) ?? null;
...
export async function assertStatusPanel({ page, check, disclosureAvailable }) {
  ...
  if (disclosureAvailable) {
    const facts = liveDisclosureFacts();
...
export async function assertHarnessesPanel({ page, check }) {
  ...
  check((await panel.locator("[data-harness-card]").first().textContent()).includes("aikit client install")
    || rendered.every((row) => row.installed === "true"),
    "Harnesses: an uninstalled card names the real install path (aikit client install) — never a fake button (L4)");
  return panel;
}

export async function assertModelsPanel({ page, check }) {
...
export async function assertCredentialsPanel({ page, check, disclosureAvailable }) {
...
export async function assertSkillsPanel({ page, check, disclosureAvailable }) {
