...
| `modes` | workspace modes and tab presentation (src/workspace/mode.ts) | one compact mode switch; Factory/Expressions/Technè open as ordinary tabs and close nothing; unsaved writing rides through every switch; per-mode remembered side regions; at most one canvas; nothing above the shell; tabs as strip / vertical list / hidden with a non-reflowing reveal edge; all of it, and the Factory binding, restores on reload; no horizontal page scroll at 1000/760/640 |
