...
fn shell_snapshot(state: State<'_, AppState>) -> Result<ShellSnapshot, String> {
    state.host.lock().map_err(|_| "desktop host lock poisoned".to_owned())?
        .snapshot(BridgeCaller::ShellUi).map_err(|error| error.to_string())
}

#[tauri::command]
fn contribution_catalog(state: State<'_, AppState>) -> Result<Vec<HostedContribution>, String> {
    BridgePolicy.authorize(BridgeCaller::ShellUi, BridgeCallClass::DiscloseContributions)
...
fn factory_build_snapshot(state: State<'_, AppState>) -> Result<Option<FactoryBuildSnapshot>, String> {
    BridgePolicy.authorize(BridgeCaller::ShellUi, BridgeCallClass::ObserveFactoryBuild)
        .map_err(|error| error.to_string())?;
    ...
    replace_contribution(&state, FACTORY_BUILD_CONTRIBUTION_REF, observation.contribution)?;
...
fn dispatch_factory_action(
    ...
    BridgePolicy.authorize(BridgeCaller::ShellUi, BridgeCallClass::DispatchFactoryAction)
        .map_err(|error| error.to_string())?;
    let mut factory = state.factory.lock().map_err(|_| "Factory provider lock poisoned".to_owned())?;
    ...
    let action = observation.contribution.contribution.actions.iter()
        .find(|action| action.action_ref == emission.action_ref)
        .ok_or_else(|| format!("Factory did not advertise Action `{}`", emission.action_ref))?;
    let binding_revision = observation.contribution.contribution.provenance.revision.clone()
    ...
    let authorised = state.action_authority.lock().map_err(|_| "Action authority store lock poisoned".to_owned())?
...
fn aikit_session_spaces(state: State<'_, AppState>) -> Result<Value, String> {
    BridgePolicy.authorize(BridgeCaller::ShellUi, BridgeCallClass::ObserveSessionSpace)
        .map_err(|error| error.to_string())?;
    let aikit = state.aikit.lock().map_err(|_| "AIKit workbench lock poisoned".to_owned())?;
    let aikit = aikit.as_ref().ok_or_else(|| "native AIKit application is not available".to_owned())?;
    to_value(aikit.list_session_spaces().map_err(|error| error.to_string())?)
}

#[tauri::command]
fn aikit_session_space_read(state: State<'_, AppState>, session_space_ref: String) -> Result<Value, String> {
    BridgePolicy.authorize(BridgeCaller::ShellUi, BridgeCallClass::ObserveSessionSpace)
        .map_err(|error| error.to_string())?;
    let runtime_observation = {
        let runtime = state.aikit_runtime.lock().map_err(|_| "AIKit runtime observation lock poisoned".to_owned())?;
        runtime.as_ref().map(LocalAikitSessionSpaceHost::observe).transpose()?
    };
    if let Some(observation) = runtime_observation.as_ref() {
        replace_contribution(&state, AIKIT_SESSION_SPACE_CONTRIBUTION_REF, observation.contribution.clone())?;
    }
    let aikit = state.aikit.lock().map_err(|_| "AIKit workbench lock poisoned".to_owned())?;
    let aikit = aikit.as_ref().ok_or_else(|| "native AIKit application is not available".to_owned())?;
    to_value(aikit.read_session_space_with_runtime(
        &session_space_ref,
        runtime_observation.as_ref().map(|observation| &observation.read_model),
    ).map_err(|error| error.to_string())?)
}

#[tauri::command]
fn aikit_session_space_focus(state: State<'_, AppState>, request: SessionSpaceFocusRequest) -> Result<Value, String> {
    BridgePolicy.authorize(BridgeCaller::ShellUi, BridgeCallClass::MutateSessionSpaceFocus)
        .map_err(|error| error.to_string())?;
    let aikit = state.aikit.lock().map_err(|_| "AIKit workbench lock poisoned".to_owned())?;
    let aikit = aikit.as_ref().ok_or_else(|| "native AIKit application is not available".to_owned())?;
    to_value(aikit.focus_session_space(&request).map_err(|error| error.to_string())?)
}

#[tauri::command]
fn agent_surface_open(state: State<'_, AppState>, request: AgentSurfaceOpenInput) -> Result<AgentSurfaceReading, String> {
    BridgePolicy.authorize(BridgeCaller::ShellUi, BridgeCallClass::InteractAgentSession)
        .map_err(|error| error.to_string())?;
    let provider = state.agent_provider.as_ref().ok_or_else(|| "no native AgentSession provider is configured".to_owned())?;
    let mut current = state.agent_surface.lock().map_err(|_| "AgentSession Surface lock poisoned".to_owned())?;
    if current.is_some() {
        return Err("an AgentSession Surface is already open; close it explicitly before replacing it".into());
    }
...
fn agent_surface_send(state: State<'_, AppState>, text: String) -> Result<Value, String> {
    BridgePolicy.authorize(BridgeCaller::ShellUi, BridgeCallClass::InteractAgentSession)
        .map_err(|error| error.to_string())?;
    let mut current = state.agent_surface.lock().map_err(|_| "AgentSession Surface lock poisoned".to_owned())?;
    let surface = current.as_mut().ok_or_else(|| "no AgentSession Surface is open".to_owned())?;
...
fn agent_surface_cancel(state: State<'_, AppState>) -> Result<(), String> {
    BridgePolicy.authorize(BridgeCaller::ShellUi, BridgeCallClass::InteractAgentSession)
        .map_err(|error| error.to_string())?;
    let mut current = state.agent_surface.lock().map_err(|_| "AgentSession Surface lock poisoned".to_owned())?;
    current.as_mut().ok_or_else(|| "no AgentSession Surface is open".to_owned())?.cancel()
}
...
fn agent_surface_close(state: State<'_, AppState>) -> Result<(), String> {
    BridgePolicy.authorize(BridgeCaller::ShellUi, BridgeCallClass::InteractAgentSession)
        .map_err(|error| error.to_string())?;
    let mut current = state.agent_surface.lock().map_err(|_| "AgentSession Surface lock poisoned".to_owned())?;
    let mut surface = current.take().ok_or_else(|| "no AgentSession Surface is open".to_owned())?;
    surface.close()
}
...
#[tauri::command]
fn knowledge_search(state: State<'_, AppState>, query: String, limit: usize) -> Result<Value, String> {
    with_knowledge(&state, |knowledge| to_value(knowledge.search(&query, limit).map_err(|error| error.to_string())?))
}

#[tauri::command]
fn knowledge_read(state: State<'_, AppState>, resource_ref: String) -> Result<Value, String> {
    with_knowledge(&state, |knowledge| to_value(knowledge.read(&resource_ref).map_err(|error| error.to_string())?))
}

#[tauri::command]
fn knowledge_relations(
    state: State<'_, AppState>, resource_ref: String, depth: u8, max_nodes: usize, max_edges: usize,
) -> Result<Value, String> {
    with_knowledge(&state, |knowledge| to_value(knowledge.relations(&resource_ref, depth, max_nodes, max_edges).map_err(|error| error.to_string())?))
}

#[tauri::command]
fn knowledge_explain(state: State<'_, AppState>, resource_ref: String) -> Result<Value, String> {
    with_knowledge(&state, |knowledge| to_value(knowledge.explain(&resource_ref).map_err(|error| error.to_string())?))
}

#[tauri::command]
fn knowledge_history(state: State<'_, AppState>, resource_ref: Option<String>) -> Result<Value, String> {
    with_knowledge(&state, |knowledge| to_value(knowledge.history(resource_ref.as_deref()).map_err(|error| error.to_string())?))
}

#[tauri::command]
fn select_semantic_ref(state: State<'_, AppState>, subject: SemanticRef) -> Result<(), String> {
    state.host.lock().map_err(|_| "desktop host lock poisoned".to_owned())?
        .select(BridgeCaller::ShellUi, subject).map_err(|error| error.to_string())
}

#[tauri::command]
fn open_destination(state: State<'_, AppState>, destination: ShellDestination) -> Result<(), String> {
    state.host.lock().map_err(|_| "desktop host lock poisoned".to_owned())?
        .open_destination(BridgeCaller::ShellUi, destination).map_err(|error| error.to_string())
}

fn with_knowledge<T>(state: &State<'_, AppState>, operation: impl FnOnce(&LocalProjectKnowledge) -> Result<T, String>) -> Result<T, String> {
    BridgePolicy.authorize(BridgeCaller::ShellUi, BridgeCallClass::ObserveKnowledge)
        .map_err(|error| error.to_string())?;
    let knowledge = state.knowledge.lock().map_err(|_| "Knowledge workbench lock poisoned".to_owned())?;
    operation(knowledge.as_ref().ok_or_else(|| "no local ProjectCentral Knowledge world is configured".to_owned())?)
}
...
fn load_local_aikit() -> Result<Option<LocalAikitWorkbench>, String> {
    LocalAikitWorkbench::discover().map(Some).map_err(|error| error.to_string())
}

fn load_local_aikit_runtime() -> Result<Option<LocalAikitSessionSpaceHost>, String> {
    let Some(path) = env::var_os("OI_AIKIT_SESSION_SPACE_OBSERVATION") else { return Ok(None); };
...
fn load_local_knowledge() -> Result<Option<LocalProjectKnowledge>, String> {
    let Some(project_root) = env::var_os("OI_PROJECT_ROOT") else { return Ok(None); };
...
fn load_agent_provider() -> Result<Option<AgentProviderConfig>, String> {
    let Some(raw_argv) = env::var("OI_AGENT_PROVIDER_ARGV").ok() else { return Ok(None); };
    ...
    if argv.is_empty() { return Err("OI_AGENT_PROVIDER_ARGV cannot be empty".into()); }
    ...
    let provenance = env::var("OI_AGENT_PROVIDER_PROVENANCE").ok()
        .map(|value| vec![value])
        .unwrap_or_else(|| vec!["native O:I desktop provider configuration".into()]);
    Ok(Some(AgentProviderConfig { connection_ref, argv, cwd, provenance }))
}
...
fn replace_contribution(
    state: &State<'_, AppState>, contribution_ref: &str, contribution: HostedContribution,
) -> Result<(), String> {
...
fn replace_contribution_in(
    contributions: &mut Vec<HostedContribution>, contribution_ref: &str, contribution: HostedContribution,
) {
...
fn main() {
    ...
    if let Some(factory) = factory.as_ref() {
        match factory.observe() {
            Ok(observation) => replace_contribution_in(&mut contributions, FACTORY_BUILD_CONTRIBUTION_REF, observation.contribution),
            Err(error) => eprintln!("O:I Factory observation degraded: {error}"),
        }
    }
    if let Some(runtime) = aikit_runtime.as_ref() {
        match runtime.observe() {
            Ok(observation) => replace_contribution_in(&mut contributions, AIKIT_SESSION_SPACE_CONTRIBUTION_REF, observation.contribution),
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            knowledge_explain,
            knowledge_history,
            personal::personal_450_snapshot,
            personal::nara_daily_snapshot,
            personal::nara_save_daily,
            personal::nara_send_selection,
            personal::epi_personal_review,
            personal::epi_personal_ground,
            personal::epi_personal_proposal,
            personal::central_now_snapshot,
            personal::reject_personal_proposal,
            personal::recognize_personal_human_source,
            select_semantic_ref,
            open_destination
        ])
