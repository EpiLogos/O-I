...
fn central_now_snapshot(state: State<'_, AppState>) -> Result<Option<Value>, String> {
    ...
    let central = state
        .central
        .lock()
        .map_err(|_| "Central NOW provider lock poisoned".to_owned())?;
    let Some(central) = central.as_ref() else {
        return Ok(None);
    };
...
/// Summon M5′/Epii or M0′/Anuttara depth around the same protected Nara
/// selection. Every call re-resolves the exact episode/revision/range in the Epi
/// process before consuming authority. The shell selection remains the Nara
/// selection; returned depth never replaces its identity.
#[tauri::command]
fn epi_personal_depth(
    ...
    let epi = state
        .epi
        .lock()
        .map_err(|_| "Epi Personal provider lock poisoned".to_owned())?;
    let epi = epi
        .as_ref()
        .ok_or_else(|| "no Epi-owned local Personal provider is configured".to_owned())?;
    ...
    let (action_ref, reading) = match kind.as_str() {
        "explain" | "review" => {
        ...
        other => return Err(format!("unsupported Personal summon `{other}`")),
    };

    #[allow(unreachable_code)]
    Err(format!("unreachable Personal action {action_ref}: {reading}"))
}
...
fn personal_depth_receipt(
    ...
    let (central_return, central_now) = if return_to_central {
        ...
        if let Some(central) = central.as_ref() {
            let returned = central.return_personal_proposal(&reading)?;
            let now = central.inspect_now()?;
            (Some(returned), Some(now))
        } else {
            (None, None)
        }
    } else {
...
fn reject_personal_proposal(
    ...
    let central = central
        .as_ref()
        .ok_or_else(|| "no Central NOW provider is configured".to_owned())?;
    central.reject_return(&handoff_id, &proposal_ref)
}

#[tauri::command]
fn recognize_personal_human_source(
    state: State<'_, AppState>,
    source: String,
    destination: String,
) -> Result<Value, String> {
    ...
    let central = central
        .as_ref()
        .ok_or_else(|| "no Central NOW provider is configured".to_owned())?;
    central.promote_human_source(&source, &destination)
}
...
fn authorize_epi_action(
    ...
    let grant_ref = authorities
        ...
        .ok_or_else(|| {
            format!("no current Epi-issued Action authority covers `{action_ref}` for Nara episode `{authority_subject_ref}`")
        })?;
    let request = ActionExecutionRequest {
        ...
        authority_subject_ref: Some(authority_subject_ref.into()),
        native_owner: EPI_NATIVE_OWNER.into(),
        required_capability_ref: action.required_capability_ref.clone(),
        binding_revision,
        now_unix_ms: now,
...
fn required_epi_binding_revision(epi: &LocalEpiHost) -> Result<Option<String>, String> {
    let observation = epi.observe()?;
    Ok(observation.contribution.contribution.provenance.revision)
}
...
fn ensure_same_subject(reading: &Value, selection_ref: &str, episode_ref: &str) -> Result<(), String> {
    ...
    if returned_selection != selection_ref || returned_episode != episode_ref {
        return Err("Epi Personal depth returned identity drift from the governed Nara subject".into());
    }
    Ok(())
}
...
fn load_local_central() -> Result<Option<LocalCentralHost>, String> {
    let executable = env::var_os("OI_CENTRAL_CTRL");
    let project = env::var("OI_CENTRAL_PROJECT").ok();
    match (executable, project) {
        (None, None) => Ok(None),
...
fn required_json_string(value: &Value, pointer: &str) -> Result<String, String> {
    value
        ...
        .map(ToOwned::to_owned)
        .ok_or_else(|| format!("Epi Personal response requires string `{pointer}`"))
}
...
fn main() {
    ...
    let epi = load_local_epi()
        ...
        .ok()
        .flatten();
    let central = load_local_central()
        .map_err(|error| eprintln!("O:I local Central NOW provider unavailable: {error}"))
        .ok()
        .flatten();
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            nara_daily_snapshot,
            central_now_snapshot,
            nara_save_daily,
            nara_send_selection,
            epi_personal_depth,
            reject_personal_proposal,
            recognize_personal_human_source,
