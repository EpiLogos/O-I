...
export function NativeSearchCommand({
  ...
  async function activateResult(result: NativePaletteResult) {
    ...
    if (result.channel === 'command') {
      setStatus(`${result.label} is the canonical O:I namespace for ${result.nativeOwner}. Product arguments, mutation semantics and authority remain native-owner syntax/contracts.`);
      return;
    }
...
function productCommandResults(surfaces: SurfaceDisclosure[]): NativePaletteResult[] {
  return surfaces
    .filter((surface) => surface.canonical_namespace.length > 0)
    .map((surface) => ({
      channel: 'command',
      ...
      authority: `namespace only; version ${surface.version_command.join(' ')} · capabilities ${surface.capability_command.join(' ')} · verify ${surface.verification_command.join(' ')}`,
    }));
}
