...
assert subprocess.check_output(['git', 'log', '-1', '--format=%P']).decode().strip() == '6ecc4b460ea179e7a923800f042f404ceb65c3de'
...
# The Actions token has contents write, NOT workflow write. The connector
# separately replaces the workflow with the already-published read-only blob.
shutil.rmtree(root)
paths = [line.split(' b/', 1)[1] for line in patch.decode().splitlines() if line.startswith('diff --git ')]
paths += ['desktop/cradle/src/configuration/setupFlow.ts', 'desktop/cradle/src/configuration/setupFlowController.ts', 'desktop/cradle/src/configuration/SetupFlow.tsx', 'desktop/cradle/tests/configuration-setup.test.mjs', 'desktop/cradle/walk/configuration-setup.test.mjs', '.github/adoption-delivery']
run('git', 'add', '-A', '--', *paths)
