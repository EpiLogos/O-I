...
export function LibraryApp(){
 ...
 useEffect(()=>{if(route.id&&edition&&route.id===edition.entry.id&&!error&&!loading){lastReading.current={...route,scene:route.scene||edition.entry.scenes[0].ref,edition:String(edition.entry.revision)};document.title=`${edition.entry.title} — O:I Expressions`;if((!route.scene||!route.edition)&&(!route.edition||route.edition===String(edition.entry.revision))){history.replaceState(null,'',routeHref(lastReading.current));setRoute(lastReading.current);}}else if(!route.id){document.title='Library — O:I';}},[route,edition,error,loading]);
 ...
 return <div className="library-host">
  ...
  {edition&&heldRoute&&!error&&<Reader key={edition.entry.expression_ref+edition.entry.revision} edition={edition} route={heldRoute} visible={readerVisible} onChange={change} onLibrary={back}/>}
