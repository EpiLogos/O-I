...
function ReaderBody({edition,route,visible,onChange,onLibrary,scene}:ReaderProps&{scene:NativeScene}){
 ...
 const copy=async()=>{try{const url=new URL(location.href);url.hash=routeHref({...route,scene:selectedScene,edition:String(edition.entry.revision)});await navigator.clipboard.writeText(url.href);setMessage('Reading link copied.');}catch{setMessage('Copy the address from your browser to share this exact reading.');}};
 ...
 return <div className={`expression-reader ${readingFace?'is-reading':''} ${route.face==='full'||route.face==='detached'?'is-full-reading':''}`} ref={reader} id={visible?"main-content":undefined} role="main" tabIndex={-1} hidden={!visible} data-expression-ref={ref} data-edition={edition.entry.revision}>
  <header className="reader-header"><button className="reader-library" aria-label="Library" onClick={onLibrary}><Icon name="library"/><span>Library</span></button><div className="reader-heading"><strong>{edition.entry.title}</strong><span>{scene.title}</span></div><div className="reader-tools"><button className="read-source-action" aria-label="Read & sources" aria-pressed={readingFace} onClick={()=>onChange({face:readingFace?'face':'verso'})}><Icon name="text"/><span>Read & sources</span></button><Tool name="external" label="Copy exact reading link" onClick={()=>void copy()}/><Tool name="expand" label="Toggle fullscreen" onClick={()=>{if(document.fullscreenElement)void document.exitFullscreen();else void reader.current?.requestFullscreen?.().catch(()=>setMessage('Fullscreen is unavailable in this browser.'));}}/></div></header>
