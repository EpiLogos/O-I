...
export type PublicationRoute=Record<'ref'|'collection_ref'|'projection'|'revision'|'expression'|'expression_revision'|'expression_projection'|'expression_projection_revision'|'scene'|'depth'|'q'|'at'|'offset',string>;
...
export interface PublicModel {model:{open:(ref:string)=>any};records:PublicRecord[];select:(ref:string,projection?:string,revision?:string)=>PublicSubject;expressions:(ref:string)=>PublicForm[];stage:(form:PublicForm,scene?:string)=>PublicStage;collections:()=>PublicResource[];search:(q?:string,collection?:string)=>PublicResource[];manifestFor:(p:PublicProjection)=>PublicManifest|undefined}
