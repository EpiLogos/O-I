...
export interface Edition {entry:LibraryEntry;publication:{schema:string;expression_ref:string;expression_revision:number;composition:{expression_ref:string;revision:number;title:string;scenes:NativeScene[];entities:Record<string,NativeEntity>;relations:Record<string,{binding_ref:string;relation:{ref:string;revision:string;availability:string};from_entity_ref:string;to_entity_ref:string;provenance:unknown[]}>;selection:{scene_ref:string;entity_ref:string|null}};expression:Record<string,unknown>;presentation:Record<string,unknown>;projection:Record<string,unknown>};readings:Record<string,Reading>;source_basis:{commit:string;revision:string;path:string};standing:string}
