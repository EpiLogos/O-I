...
def write_product(
    root: Path, product_id: str, *, rows: list[dict[str, str]] | None = None,
    registry: dict | None = None,
) -> None:
    descriptor = {"schema": "oi.product-lifecycle/v1", "id": product_id}
    if registry is not None:
        descriptor["capability_matrices"] = registry
    descriptor_path = root / ".oi/product.json"
...
class CollationTests(unittest.TestCase):
    ...
    def test_selects_one_suite_profile_and_receipts_every_native_matrix_carrier(self) -> None:
        temporary, manifest, workspace = self.fixture()
        with temporary:
            root = workspace / "Quaternal-Logic"
            native_data = root / "docs/native-field.json"
            native_rendering = root / "docs/NATIVE-FIELD.md"
            partition = root / "docs/native-field-m0.json"
            native_data.parent.mkdir(parents=True, exist_ok=True)
            native_data.write_text('{"schema":"native/1"}\n', encoding="utf-8")
            native_rendering.write_text("# Native field\n", encoding="utf-8")
            partition.write_text('{"schema":"native-partition/1"}\n', encoding="utf-8")
            registry = {
                "schema": "oi.product-capability-sources/v1",
                "primary_id": "native-field",
                "matrices": [{
                    "id": "suite-profile", "role": "suite-profile",
                    "protocol": "ql-capability-matrix/1", "suite_catalogue": True,
                    "carriers": {
                        "manifest": "ProjectCentral/user/capability-matrix.json",
                        "records": "ProjectCentral/user/capability-matrix.csv",
                    },
                }, {
                    "id": "native-field", "role": "native-deep-subsystem",
                    "protocol": "native/1", "suite_catalogue": False,
                    "carriers": {"data": "docs/native-field.json", "rendering": "docs/NATIVE-FIELD.md"},
                    "partitions": ["docs/native-field-m0.json"], "partition_protocol": "native-partition/1",
                }],
            }
            write_product(root, "quaternal-logic", registry=registry)
            output = Path(temporary.name) / "out"
            result = self.invoke(manifest, workspace, output)
            self.assertEqual(result.returncode, 0, result.stderr)
            catalogue = json.loads((output / "catalogue.json").read_text())
            product = next(item for item in catalogue["products"] if item["id"] == "quaternal-logic")
            field = product["capability_matrices"]
            self.assertEqual(field["selected_suite_catalogue_id"], "suite-profile")
            self.assertFalse(field["fallback"])
            self.assertEqual(len(field["matrices"]), 2)
            native = next(item for item in field["matrices"] if item["id"] == "native-field")
            self.assertEqual(native["partition_protocol"], "native-partition/1")
            self.assertEqual(len(native["carriers"]["data"]["sha256"]), 64)
            self.assertEqual(len(native["partitions"][0]["sha256"]), 64)
            self.assertNotIn("cap.quaternal-logic.native", [
                record["id"] for record in catalogue["records"] if record["owner_id"] != "quaternal-logic"
            ])

    def test_rejects_ambiguous_suite_profiles(self) -> None:
        temporary, manifest, workspace = self.fixture()
        with temporary:
            registry = {
                "schema": "oi.product-capability-sources/v1", "primary_id": "one",
                "matrices": [
                    {"id": name, "role": "suite-profile", "protocol": "ql-capability-matrix/1",
                     "suite_catalogue": True, "carriers": {
                         "manifest": "ProjectCentral/user/capability-matrix.json",
                         "records": "ProjectCentral/user/capability-matrix.csv",
                     }} for name in ("one", "two")
                ],
            }
            write_product(workspace / "Quaternal-Logic", "quaternal-logic", registry=registry)
            result = self.invoke(manifest, workspace, Path(temporary.name) / "out")
            self.assertEqual(result.returncode, 2)
            self.assertIn("exactly one suite_catalogue=true", result.stderr)
