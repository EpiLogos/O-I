...
Lanes executed 26 September on the local arrangement; all workers returned and their diffs were inspected before integration.

### Landing points

| Lane | Repository / seat | Head | Content |
|---|---|---|---|
| A | ai-kit `worktrees/env-1/ai-kit`, branch `convergence/command-encounter-20260926` | `527c6fdd` | Grouped encounter with all 76 old roots as same-handler aliases (parity-tested), inert bare/`--json` orientation, `act discover/describe/invoke`, generated `system commands --json`, SkillSet repertoire resolution in Direct-agent preparation |
| A | Central `ctrl` `737582c45a98` / Actuation `e5225dc` | — | Task-oriented help only; routes and envelopes unchanged |
| B | ai-kit (same branch) | `426b19e3` | World-first entry with numbered next steps, agent-work stage machine over a refusing-by-default backend seam, keystroke discipline incl. a pre-existing query-crash repair; 324/324 + clippy `-D warnings` |
| C | O-I local main `ad7b29ce`, `b7f13cc2` | — | `oi world/search/act/agent/work/explain/ui` with byte-compatible legacy routes (7/7 parity), SkillSet-first creator with Save-and-start-Direct compound, team formation through `central.agent-set.*`, standing-distinct cards; tsc clean, 40+ node tests, 137+14 kernel tests |

Combined ai-kit gate: 1229 tests run, 1228 passed; the one failure is `knowledge_project_code_scope_real` (installed gitnexus binary rejects its own argv; proven pre-existing by stash-replay during the resident-custody lane). oi-cli: 18/19 targets green; `desktop_install` needs a built app bundle (pre-existing). Acceptance package installed through `oi update --apply --candidate` (receipts swapped atomically): aikit `426b19e34ade`, ctrl `737582c45a98`, oi `b7f13cc2c94c`.

### First joined slice — proven live on the installed cut

    oi world / bare aikit orientation (X1, degraded readings named)
      → task-language search + praxis discovery: verification/closeout practices found (X2)
      → express probe Agent (SkillSet `central-engineering` + individual exception) — refused first without ratified worlds, proving the gate
      → review/accept via human native token, exact revision+digest CAS; truncated digest refused with agent_profile_review_stale (X4)
      → prepare → agent-session/direct-f7c751…, provider_started:false, withheld_members:[] (X5)
      → readback + agent-session-find by request-id reconcile (X8 primitive)
      → open on the resident owner: real pi body spawn, journal event direct-agent-context-submitted BEFORE first turn (X6)
      → real model reply quoting purpose and all six delivered practices verbatim; second turn applied central-capability-discovery correctly (ctrl actions --json reads, never installs)
      → Return via projectcentral.now.return (native receipt)

Probe Agent and probe agent-set removed by CAS after proof.

### Live failures the slice exposed and what they proved

- Stale machine provider config (`pi` → missing `Work/epi/bimba-portable/pi-mcp-bridge`): launch failed with `agent_session_host.transport_closed` while the saved source stayed intact — X10's "Saved; not running" proven with a real provider failure, not a fixture. Worked around with a minimal `pi-plain` provider via `encounter-configure`.
- Version skew: the resident owner kept the pre-install binary and rejected the new `withheld_members` binding field; graceful owner restart (no residents attached) is part of the update path — recorded here because `oi update` keeps running binaries by design.
- Every Agency-policy provider correctly refuses profile-based Direct work ("needs a real Agency/WorldBinding") — the Direct-vs-Agency boundary holds.
