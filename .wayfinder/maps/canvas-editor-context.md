...
### Integration (20 September 2026)

Reviewed against and reconciled with current mains; landed AIKit-first. AIKit #363 merged (squash `a5531dc`). The integration walk uncovered a real defect on ai-kit main: after the O-I #376 fold, `encounter-start`/`encounter-model-exec`/`encounter-task-exec` self-spawns assumed the pre-fold verb shape, so no resident could start from the folded binary; [AIKit #370](https://github.com/EpiLogos/ai-kit/pull/370) repairs it shape-aware. O-I #441 combined revision `346af0e` (branch head `0b9f4849` + main; PR files byte-identical) passed the whole receipt suite locally. New native walk `desktop/cradle/walk/scenarios/canvas-context.mjs`: 28/28 against the real candidate in a disposable ground/home — exact repeated-text selection (emoji proves UTF-16), non-modal Project-scope preparation, cue/reveal/remove, native Vāk Resolve, stale-source refusal with zero dispatch, explicit adoption into the session, and one real provider Return (existing pi harness) with the structured owner-recorded receipt. The installed suite still runs a pre-canvas AIKit; the standard `oi install` route needs the Work/ai-kit checkout reconciled to main (an active lane holds it). [Exact receipt](../../desktop/cradle/walk/artifacts/CANVAS-EDITOR-CONTEXT-2026-09-20.md) carries commands and SHAs.
