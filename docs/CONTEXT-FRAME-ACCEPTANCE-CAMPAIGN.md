...
- **Open:** (a) the `00/00` GUI-launch leg — the Desktop lifecycle is proven to install, restart and remove a real bundle, but one integrated encounter through the launched app has not been observed; (b) §5 grant expiry — access today ends only by revocation, no TTL exists; (c) the §5 case-6 shared-ground half — disjoint state between two machines is proven, a shared related ground is not; (d) human-observed results — every executed case is an agent return awaiting the owner's Recognition; none of it is human acceptance.
