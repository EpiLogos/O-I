# O:I Desktop + Agent-Native Surface Ledger — current-main closure cut

**Status:** #155 W9/W11 / #111 deterministic closure ledger  
**Date:** 2026-09-02  
**Carrier:** O:I PR #164 (`converge/oi155-inhabitation-current`)  
...
## Exact native-owner source cut

| Product | Exact source revision | Current relation consumed |
|---|---|---|
...
The same source revisions are recorded in `surfaces.json` and `suite/mainline.json`. `suite/manifest.json` remains the immutable earlier released-artifact snapshot rather than being rewritten as present source truth.

## Final application-Surface disposition

| Capability / relation | Native owner | Desktop / local O:I | Site / Explore / SharedField | Structured Agent / alternate native | Evidence / standing |
|---|---|---|---|---|---|
| Recursive World + provenance propagation | Central | **IMPLEMENTED** through current Project/CurrentWorld/Knowledge application bodies; presentation never becomes source identity | explicit Projection only | Central/AIKit refs and bounded Context resolution | Central #98 merged as `f3fde4d5…`; current `ctrl/src/world.rs` remains on exact Central main |
| Authored/revisioned AgentSet + effective participant target | Central source; AIKit effective Knowledge/participant resolution; Actuation actual co-actuation | **IMPLEMENTED** typed human/Agent/AgentSet picker; authored set is not UI state | SharedField membership/addressing remains distinct from AgentSet identity | AIKit #134 merged; Central `central.agent-set/v1`; unavailable members do not rewrite authored set | Central #97/#98 and AIKit #133/#134 are completed native-owner work |
| Project/files/authored Ground/Wiki/Knowledge/Flow | Central + AIKit | **IMPLEMENTED**; authored source remains distinguishable from Agent-maintained/derived knowledge | only selected/projected material | stable refs/read/search/relations/ContextSource; native `ctrl`/AIKit surfaces | current W7/W8 + owner exact-main preflight |
| Canonical Agency / AgentSession conversation | Actuation + AIKit | **IMPLEMENTED** sidecar/session relation | no automatic public projection | canonical AgentSession, Herdr/tmux/cmux/harness-native alternate Surfaces | inherited SessionSpace/AgentSession floor + current #154/#155 relations |
| Typed `To:` / `@` addressing | AIKit participant-target grammar + O:I presentation | **IMPLEMENTED** `participant-composer-model.mjs`; local query/highlight state remains presentation-only | **IMPLEMENTED** where target/membership is available | `aikit.participant-address/v1`; addressing carries no invocation authority | AIKit #134 + O:I composer/addressing tests |
| Semantic Activity | native semantic/event owners; O:I application composition | **IMPLEMENTED** global/session-readable Activity and Observatory presentation | projected only where audience permits | structured Activity + raw/native trace refs; Actuation/AIKit native traces remain available | Actuation Activity contract; `shared-field/activity.*`; W11 return Activity |
| Notification | transient O:I/native presentation projection | **IMPLEMENTED** as `oi.notification/v1`; exact subject/activity deep-link; no notification truth store | available where SharedField/watch policy permits | native personal/OS notification remains Central/provider operation | `notificationFromActivity` tests prove optional projection |
...
| Explore / WorldPresentation / Projection | O:I SharedField/Explore; source owner remains native | **IMPLEMENTED** shared renderer-neutral ExploreSurface | **IMPLEMENTED** same application model + live SpaceTimeDB | structured Agent consumes same refs/read model; no DOM scraping | W9 tests + W11 live reducer→subscription return |
| Hosted authoring/return authority | SharedField + Projection/source owner | **IMPLEMENTED** returned shared application reading | contributor-authorised rev `1→2`; public reader mutation refused | same semantic World/Projection refs and source revision | W11 `application-return-live-acceptance.ts` |
| Herdr / Hyprland SessionSpace environment | AIKit | **IMPLEMENTED owner/provider state; ALTERNATE-NATIVE provider UI** | N/A | Herdr rich reference, tmux thin portable, cmux optional | AIKit #144/#148 accepted; isolated provider loss preserves SessionSpace/AgentSession identity |
| Workcell / OpenSandbox nested material World | Workcell | **IMPLEMENTED** as material/System/Lower reading where supplied; O:I is not runtime owner | explicit Projection only | Workcell public client/provider/control surfaces | deterministic current main; actual OpenSandbox/provider execution remains **PHYSICAL** #97 |
| Agency Gateway same Agency/session across loci | AIKit + Actuation + Workcell | **IMPLEMENTED deterministic application/service relation** | authorised connector/gateway Surface only | structured ecology/connection semantics; provider endpoint is not Agency/session identity | current Workcell Gateway service body + AIKit/Actuation contracts; primary↔second-machine continuity is **PHYSICAL** |
| Native Omarchy / Quickshell host | O:I #159 consuming AIKit environment/provider facts | **ALTERNATE-NATIVE** first-party host beside full Tauri desktop | N/A | deterministic bridge/current-world/action reading | #159 deterministic host already on O:I main; real login/reload/host experience remains **PHYSICAL** |
| QL/Epi integral M0′↔M5′ and deep M′ instruments | QL-MEF/Epi | **ALTERNATE-NATIVE / PARTIAL according to accepted producer**; preserve predecessor detailed ledger | explicit Projection only; protected bodies stay local | native structured refs/actions/instrument surfaces where current | **MOVING-OWNER-EXCEPTION**; unfinished QL/Epi owner work does not become a generic O:I/#97 blocker |

## Parity and degraded/security closure
...
- **version/basis conflict:** V7 rejects stale human-Ground overwrite, AIKit VersionedWorld rejects Project rebinding, and Factory rejects P5 returned under another P4 ContextResolution;
- **renderer containment:** rich/browser contributions do not receive ambient shell/fs/process/network/secret authority;
- **credentials:** raw secret material remains outside ordinary application/configuration state;
- **material absence:** unavailable Workcell/provider paths remain availability facts rather than substitute semantic Worlds;
- **QL/Epi degraded state:** predecessor ledger preserves fixture/derived-only truth and refuses false live-now claims.
...
```text
Central recursive World + authored AgentSet + source/Intent
    ↓
...
Actuation Activity / canonical Agency and AgentSession ecology
    ↓
Activity → optional Notification → explicit unresolved Attention
    ↓
O:I System + Observatory + shared Explore application
    ↓
authorised hosted Projection operation
    ↓
same World / Projection revision returns to browser + desktop + structured Agent
    ↓
Workcell / Gateway / Omarchy material relations remain exact and truthfully physical-gated
```

If the current exact-head remote suite is green, the selected deterministic #155/#111 body is ready to return to native O:I main. The next legitimate uncertainty is the #97 human/material World, not another branch-only semantic programme.
