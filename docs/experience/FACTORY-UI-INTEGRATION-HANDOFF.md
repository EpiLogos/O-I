...
- **Centre** (`contributions/factory/FactoryCentre.tsx` over `desk/deskModel.ts`): module stores for the centre view, the opened-Run detail and the last desk-chosen Run — display preferences, never a run database. Desk renders `desk/DeskBoard.tsx`; an opened Run renders `desk/DeskRunDetail.tsx` around the owner's own `BuildSurface`; Tasks renders the shared `AgentChat` in a full-canvas presentation (`factory-chat-full`): context strip with Project, Run link when bound and an honest Direct-conversation marker; comfortable reading measure for prose, wider lane for code; transcript dominant; composer anchored inside the pane's own bottom edge (the fresh state's welcome reads from the top). The head carries the conversation's own controls only — new chat and history: the panel's chrome (inside-out, expand right region) stays on the panel face (`AgentChat variant="centre"`). Type across Factory follows the shell's Central-mode ladder (rows at nav, secondary at ui). It is the same shared session observer, composer and draft — not the old sidebar widget in a wider div, and not a second implementation.
...
- **Shell geometry reconciled (same day, follow-up round)**: the right agent panel had two competing positioning regimes — in-flow (`position:relative`) in component modes but with the old absolute regime's row spacer still reserving its width, so the Central/Factory panel floated 320px off the right edge with a phantom agent-ground band ("two boxes"), and open/close moved the centre and the background in opposite directions. One law now: the component-mode panel takes its layout room itself (no spacer); the canvas modes' panel floats absolute with the spacer reserving under it (glass untouched); `depth-full` floats over the centre in every mode. Resize UX: drags track 1:1 (the geometry hook yields to a live drag — mid-drag renders can no longer write stale targets, and unchanged targets no longer restart animations), double-click or Home resets a region to its tier default, arrow keys step, and a live width badge shows the value while dragging. Base readability: the shell's type ladder reads one step up (eyebrow 10 → prose 15), rows/controls/hit targets +2px, sidebar icons 16px (`shell.css`, `geometry.ts`, `DesktopShell.tsx`, design-system `tokens.css`).
...
### 2026-09-22 successor entry repair (#65/#201; O-I #476)

The installed-bootstrap successor discovers native Factory placements without manual source entry and keeps the carried conversation's scope separate from project browsing. The next presentation revision makes the Agent roster the entry, moves creation/Skills and exact source details behind explicit controls, distinguishes Central personal ground from the Central product project, and removes unrelated project-ledger scans from the scope picker. Tasks use the native SessionSpace label before falling back to a bounded purpose excerpt. Neither labels nor browse selection rewrite session identity.

Run Activity now reads the selected Run's native execution traces, preserving separate executions, spans/attempts and event inputs/results. With no Run selected, accompanying conversation activity remains explicitly labelled as conversation activity. This is a consumer repair; it does not establish dispatch, fill missing owner events, or constitute independent verification. `/model` opens the existing native model control in a connected chat; other slash commands retain the connected harness's actual support. The catalogue is labelled as known models, not promised launch availability.

Frontend production build passes. Live installed replay, provider/model controls, actual Factory dispatch, joined recovery and human acceptance remain separately required under the existing campaign record; no parent obligation closes from this increment.


The installed c3088244 walkthrough retained a presentation failure: long Agent purposes overflowed fixed-height roster rows. The successor uses the design system's title/state row children, adds panel spacing, and suppresses the extra-plane scrollbar per shell law. That failed screenshot remains in the campaign task.

New-chat harness selection now uses the native configured body list and an explicit kernel override checked before creating state. The existing held/default first-send behavior remains available. “Configure model before sending” opens the same native conversation without sending its draft; it is an optional path to the session's native model controls. Mint refusal no longer borrows another session's historical Agency source. Its exact source/admission is returned by AIKit. The obsolete fallback-reuse test was removed with that behavior; the real installed settings proof now requires fresh native admission, exact selected harness, a resident process, and preservation of the person's held default. Existing mint absence/refusal and default-choice checks remain. No provider inference is claimed by configuration proof.

The next binding repair scopes root conversation discovery by its actual native local World path. Installed-owner read-only replay retained one current and one foreign root World, preserved the current row exactly and excluded the foreign row. An invalid explicit harness was rejected before any native SessionSpace allocation. Parked draft transfer now waits for native CAS acceptance and matches the exact provisioned AgentSession; model preparation opens its existing native controls without sending. A final provider-open refusal returns the real prepared conversation and uses existing reconnect; pre-mint failure remains a retained failure and does not authorize borrowing another session's Agency. Whole-path installed replay remains required.
