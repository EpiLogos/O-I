...
The primary integrator owns shell placement, shared session interaction, native joins and the joined walk. The owner's existing Terra/Luna delegation authorisation permits bounded parallel work where independent: returned-template variants, Git projection, roster/picker work and acceptance coverage. Partition writes, return exact evidence and converge through one integration lane. No concurrent writers on the same mutable subject.
