...
| T2 · Technè opens onto Instrument 0 as the wiki web — Central and its projects' registers, no material required (owner direction 2026-09-19) | the wiki.json readings (WikiMapNavigator's law, extracted to `techne/wikiReading.ts`) | owner file seam (`files_list`/`file_read`) | `techne/WikiWebBody.tsx` as Instrument 0's body; the material scene + lens host stay mounted as the summoned depth (surfacing when material exists; the navigator's Add-to-instrument still lands) | entries open their knowledge page through the frame's ordinary open path | `walk/techne-instrument-tabs-probe.mjs` (M0 = wiki web, no material gate; tab/state grammar unchanged) | **landed 2026-09-19** |
