# SharedField × QL/Epi Integration Wayfinder
...
The QL/Epi field has crossed an integration threshold:
...
Keep these identities and offices distinct:

```text
native source / authored artifact
    owned by Central / Project / product-native source owner

Wiki / Knowledge
    persistent authored/derived knowledge topology and navigation

HTML / WorldPresentation / rich Surface
    authored carrier or presentation form, according to its native contract

Projection
    explicit audience-scoped shared representation with its own revision/provenance

SharedField
    relational environment of Participants, Contributions, Encounter, Watch and Return

SpaceTimeDB
    hosted materialisation of SharedField application state, never semantic identity
...
```text
M0′  Bimba / source / Wiki
M1′  formal-harmonic-topological instrument
M2′  spectral-musical-symbolic/aperture instrument
M3′  form-transcription-clock instrument
M4′  Nara / lived personal instrument
M5′  canonical Epii / same AgentSession
```

Retain current writing, files, Wiki/Knowledge, Factory, Agent/session, tabs/splits/pop-outs and particle-field bodies. No replacement shell or second Agent loop.

Every appropriate selected object should be able to expose native owner Actions such as open, inspect, relate, operate, project/share, watch, contribute and return **only where its owner and disclosure policy actually permit them**.

The shared HTML template/output family in #289 is part of this bridge: structured HTML can be a canonical authored artifact or a rich presentation depending on its native form. Publication is a separate Projection act.
...
3. project real Central/ProjectCentral Wiki objects without transferring Wiki/source ownership;
4. project at least one real curated HTML artifact through an audience-scoped representation;
5. make browser Explore and O:I desktop resolve/open the same semantic refs;
...
Vāk / C′ performance records and representations
T/T′ reports that have entered durable recognised knowledge
recognised named praxis / Methods
Epii-attributable outputs and explanations
...
The first complete content journey should prove this relation rather than another demo fixture:

```text
real native source / curated HTML artifact
        ↓ authored/owned
...
explicit Return / proposal
        ↓ native owner CAS / Recognition
new source or Wiki revision
        ↓ optional reprojection
```

Requirements:

- stable source/artifact identity and source revision survive projection;
- Projection revision remains distinct from source revision;
- embedded/private HTML document state is filtered according to the publication contract rather than merely hidden by CSS;
- arbitrary hosted HTML/JS receives no ambient desktop, filesystem, session or Action authority;
- Wiki edges, SharedField contribution relations and learned KnowledgeRoutes remain distinct even when rendered together;
- desktop and web may present the same object differently without minting another semantic object.

## 5. Two-machine topology

Use the second machine as the first real server/Workcell specimen rather than waiting for final acceptance:

```text
PRIMARY WORKSTATION
  O:I desktop / authoring / local World
...
SECOND WORLD / BROWSER / AGENT
  ...
  Explore / Encounter / Contribution
```

Tailscale/Fabric provides reachability. SSH provides administration where selected. Gateway provides application-level Agency/session communication. SpaceTimeDB provides live SharedField state. Workcell owns material service lifecycle. None substitutes for another.

## 6. Joined acceptance sequence

This bridge is complete only when one real journey succeeds across exact current revisions:

```text
1. author/open a real Wiki-linked or curated HTML object on the primary World
2. project only the selected representation to the hosted SharedField
3. discover/open it from an independently grounded second identity/world
4. inspect provenance and bounded Wiki/local-whole relations
5. Contact / Watch / join a SharedField under explicit authority
6. contribute a change, response or related artifact
7. optionally involve an authorised Agent through the existing Gateway/session path
...
- repository/CI contracts prove deterministic and hosted-provider facts;
- the two real machines prove material/network/service behaviour;
- actual model/harness use proves provider Agency behaviour;
- the human owner supplies visual/interaction/Recognition evidence.

## 7. Scheduling law

From 2026-09-14 forward:
...
> The living M′/Epii/Nara instrument is locally inhabitable, and every appropriate part of that world already has an explicit native route into selective co-internal/shared experience through the same refs, owners, authority and provenance.
