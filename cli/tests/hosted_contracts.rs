...
fn factory_source_validates_registers_and_reads_back_without_runtime_activation() {
    ...
    let valid = successful(run(
        home.path(),
        &["contribution", "validate", path, "--json"],
    ));
    ...
    let shown = successful(run(
        home.path(),
        &["contribution", "show", "oi.contribution/factory"],
    ));
    assert_eq!(shown, valid["document"]);
    assert_eq!(
        successful(run(home.path(), &["contribution", "list"])),
        json!([shown])
    );
    assert_eq!(
        successful(run(home.path(), &["contribution", "register", path])),
        registered
    );
}
...
fn changed_source_and_escaping_entry_are_refused_before_registration() {
    ...
    let mut document =
        hosted::read(&repo().join("desktop/cradle/src/contributions/factory/contribution.json"))
            .unwrap();
    let source = directory.path().join("contribution.json");
    fs::write(
        directory.path().join("descriptor.tsx"),
        b"changed reviewed entry",
    )
    .unwrap();
    write(&source, &document);
    let output = run(
        &directory.path().join("home"),
        &["contribution", "register", source.to_str().unwrap()],
    );
    ...
    let output = run(
        &directory.path().join("home"),
        &["contribution", "validate", source.to_str().unwrap()],
    );
    assert!(!output.status.success());
...
fn contribution_schema_and_compilation_refuse_collisions_and_runtime_urls() {
    ...
    document["surfaces"]
        .as_array_mut()
        .unwrap()
        .push(original["surfaces"][0].clone());
    assert!(hosted::validate(&document).unwrap_err().contains("repeats"));
    let mut document = original.clone();
    document["entry"] = json!("https://external.invalid/entry.ts");
    assert!(hosted::validate(&document).is_err());
    let mut document = original.clone();
    document["runtime_loader"] = json!("module.js");
    assert!(hosted::validate(&document).is_err());
    assert!(
        hosted::compile_registry(path.parent().unwrap(), &[path.clone(), path.clone()])
            .unwrap_err()
            .contains("duplicate")
    );
}
...
fn world_presentation_native_roundtrip_is_lossless_and_accepted_by_shared_consumer() {
    ...
    successful(run(
        directory.path(),
        &["presentation", "validate", source.to_str().unwrap()],
    ));
    let registered = successful(run(
        directory.path(),
        &["presentation", "register", source.to_str().unwrap()],
    ));
    assert_eq!(registered["activation"], "registered-data");
    let shown = successful(run(
        directory.path(),
        &["presentation", "show", "presentation:contribution-contract"],
    ));
...
fn native_registration_roundtrips_through_the_production_desktop_explore_renderer() {
    let output = Command::new("node")
        .current_dir(repo())
        .args([
            "desktop/cradle/tests/hosted-presentation-native.mjs",
            env!("CARGO_BIN_EXE_oi"),
        ])
        .output()
        .unwrap();
    assert!(
...
fn immutable_registration_refuses_same_revision_change_and_downgrade() {
    ...
    value["title"] = json!("Changed title");
    write(&source, &value);
    assert!(
        hosted::register(&registry, &source, hosted::PRESENTATION_SCHEMA)
            .unwrap_err()
            .contains("immutable")
    );
    ...
    assert_eq!(
        hosted::show(&registry, "presentation:contribution-contract").unwrap()["revision"],
        2
    );
}
...
fn competing_revision_registration_never_rolls_back_a_newer_revision() {
    ...
    for revision in 1..=12 {
        ...
        jobs.push(std::thread::spawn(move || {
            hosted::register(&root, &source, hosted::PRESENTATION_SCHEMA)
        }));
    }
    for job in jobs {
...
fn adopted_catalogue_namespace_reaches_real_desktop_service_registration() {
    ...
    catalogue["surfaces"]
        ...
        .unwrap()["native"]["namespace"] = json!("aikit-reviewed");
    let source = directory.path().join("surfaces.json");
    write(&source, &catalogue);
    let output = Command::new(env!("CARGO_BIN_EXE_oi"))
        .args(["catalogue", "adopt", source.to_str().unwrap()])
        .env_clear()
        .env("HOME", directory.path())
        .env("OI_HOME", directory.path())
...
fn catalogue_service_child() {
    ...
    assert!(
        census.current_world.error.is_none(),
        "{:?}",
        census.current_world.error
    );
    assert_eq!(
        oi_cradle_kernel::system_composition::namespace_for(&census, "ai-kit").unwrap(),
        "aikit-reviewed"
    );
