...
fn pickup_projects_guardian_set_into_harness_trees_with_receipts() {
    ...
    // `oi-suite-operator` projects SKILL.md and its receipt only.
...
case "${1:-}:${2:-}" in
  ...
  procedure:list)
    ...
    exit 0 ;;
  procedure:diff)
    printf '%s\n' "{\"schema\":1,\"ok\":true,\"context\":{},\"data\":{\"diff\":\"procedure prc-guardian (d1) — 8 edits\\nnote: adopt 2 valid Agent Skill(s) from $SRC into the personal registry\"}}"
    exit 0 ;;
  procedure:undo)
    for d in "$ORIG"/*/; do
      name=$(basename "$d")
    ...
    printf '%s\n' '{"schema":1,"ok":true,"context":{},"data":{"procedure":"prc-guardian","undone":8}}'
    exit 0 ;;
  set:show)
    printf '%s\n' '{"schema":1,"ok":false,"context":{},"error":{"code":"set.missing","message":"no such set"}}'
    exit 3 ;;
...
fn sync_refreshes_a_stale_adopted_tree_through_aikit_procedures() {
    ...
    // The re-adopt previews once per capsule husk the undo left behind and
    // once more over the cleared tree before the apply lands.
