...
fn model_default_override_routes_hold_review_apply_and_readback_to_the_same_owner() {
    ...
    let native_provider =
        std::env::var("OI_NATIVE_PI_PROVIDER").expect("actually observed Pi provider");
    ...
    assert!(
        !work
            .path()
            .join("aikit/state/config/model-defaults.json")
            .exists(),
        "holding must not apply the owner setting"
    );
    ...
    assert_eq!(
        applied["changeset"]["status"], "verified",
        "applied={applied} readback={reading}"
    );
    ...
    assert_eq!(
        run(&["config", "show", setting, "machine", "--json"])["native"]["effective"]["value"],
        json!({})
    );
    ...
    assert!(
        ...
        "an invalid explicit owner must not fall back to the installed owner"
    );
