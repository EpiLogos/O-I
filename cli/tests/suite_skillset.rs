...
fn oi_and_central_without_aikit_have_legal_direct_projection_path() {
    ...
    assert!(effective
        ...
        .iter()
        .any(|gap| gap.owner_product == "AIKit"));
    assert!(effective
        .skills
        .iter()
        .all(|relation| relation.resolution_mode == SkillResolutionMode::OiDirectProjection));
...
fn ordinary_base_operator_does_not_receive_root_extension_authoring_skills() {
    ...
    assert!(!base
        .expected_native_skills
        .iter()
        .any(|gap| gap.owner_product == "Actuation"));
}
...
fn root_profile_is_expanded_but_skill_availability_does_not_grant_capability_or_action_authority() {
    ...
    let root_skill = manifest
        .skills
        .iter_mut()
        .find(|skill| skill.skill_ref == "central:skill:connector-authoring")
        .unwrap();
    ...
    authority.capability_grants.insert(
        "central.connector.author".into(),
        CapabilityGrantState::Granted,
    );
    authority.action_authorizations.insert(
        "central.connector.register".into(),
    ...
    assert_eq!(
        authoring.capability_states["central.connector.author"],
        CapabilityGrantState::Granted
    );
    assert_eq!(
        authoring.action_states["central.connector.register"],
        ActionAuthorizationState::Denied
    );
    assert!(root
        .expected_native_skills
        .iter()
        .any(|gap| gap.owner_product == "Actuation"
            && gap.requiredness == Requiredness::IfProductInstalled));
}
...
fn unavailable_native_product_skill_is_disclosed_as_gap_without_fake_capability() {
    ...
    assert!(!root
        .skills
        .iter()
        .any(|skill| skill.source_repository == "EpiLogos/Actuation"));
}
...
fn availability_and_authority_are_three_separate_axes_by_default() {
    ...
    let skill = manifest
        .skills
        .iter_mut()
        .find(|skill| skill.skill_ref == "oi:skill:operate-suite")
        .unwrap();
    ...
    assert_eq!(
        skill.capability_states["oi.package.manage"],
        CapabilityGrantState::NotEvaluated
    );
    assert_eq!(
        skill.action_states["oi.package.install"],
        ActionAuthorizationState::NotEvaluated
    );
}
