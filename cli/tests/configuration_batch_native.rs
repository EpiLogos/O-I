...
fn native_resolution_batch_preserves_order_names_and_per_address_failure() {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_oi"))
        ...
        .spawn().unwrap();
    child.stdin.take().unwrap().write_all(serde_json::to_vec(&request).unwrap().as_slice()).unwrap();
    ...
    assert_eq!(rows[0]["native_reading"]["reading_digest"], rows[1]["native_reading"]["reading_digest"]);
    assert!(!rows[0]["native"].is_null(), "AIKit must actually disclose native skill state: {}", rows[0]);
