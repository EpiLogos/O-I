...
pub const EFFECTIVE_SKILLSET_SCHEMA: &str = "oi.effective-skillset/v1";
...
/// justified by an Actuation positional WorldBinding/root determination.
...
pub struct DirectProjectionReceipt {
    ...
    /// Deterministic drift fingerprint only; this is not a security/trust digest.
...
pub fn resolve_profile(
    ...
    installed_products: &BTreeSet<String>,
    ...
    for (skill_ref, requiredness) in members {
        ...
        let owner_installed = installed_products.contains(&skill.owner_product);
        let availability_required = match requiredness {
            Requiredness::Required => true,
            Requiredness::IfProductInstalled => owner_installed,
            Requiredness::Optional => false,
        };
        if availability_required && availability != SkillAvailability::Available {
            degraded = true;
        }
    ...
    let expected_native_skills = manifest
        ...
        .cloned()
        .collect::<Vec<_>>();
    for expectation in &expected_native_skills {
        let required = match expectation.requiredness {
            Requiredness::Required => true,
            Requiredness::IfProductInstalled => {
                installed_products.contains(&expectation.owner_product)
            }
            Requiredness::Optional => false,
        };
        if required {
    ...
    Ok(EffectiveSkillSet {
        schema: EFFECTIVE_SKILLSET_SCHEMA.to_owned(),
...
pub fn materialise_direct_projection(
    ...
    if !matches!(skill.owner_product.as_str(), "O:I" | "Central") {
    ...
    if existed_before {
            ...
            || receipt.destination != destination.to_string_lossy()
        {
            return Ok(conflict(
                Some(receipt),
                "existing projection receipt belongs to another Skill or destination; preserved",
            ));
        }
        ...
        if current_digest != receipt.generated_digest {
            return Ok(conflict(
                Some(receipt),
                "derived projection has local edits; preserved without overwrite",
            ));
        }
        ...
        return Ok(conflict(
            read_projection_receipt(&receipt_path).ok(),
            "orphaned O:I projection receipt exists; preserved",
        ));
    }
...
pub fn remove_direct_projection(
    ...
    if !destination.exists() || !receipt_path.exists() {
        return Ok(conflict(
            None,
            "projection file/receipt pair is incomplete; preserved",
        ));
    }
    let receipt = read_projection_receipt(&receipt_path)?;
    let current = fs::read(destination)
    ...
    if receipt.skill_ref != skill_ref || stable_digest(&current) != receipt.generated_digest {
        return Ok(conflict(
            Some(receipt),
            "projection is not an unmodified O:I-owned derived copy; preserved",
        ));
    }
...
fn read_projection_receipt(path: &Path) -> Result<DirectProjectionReceipt, String> {
    let bytes =
        fs::read(path).map_err(|error| format!("could not read projection receipt: {error}"))?;
