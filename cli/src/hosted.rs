...
const CONTRIBUTION_CONTRACT: &str =
    include_str!("../../schemas/oi.native-source-contribution-v1.schema.json");
const PRESENTATION_CONTRACT: &str =
    include_str!("../../schemas/oi.world-presentation-v1.schema.json");
...
pub fn read(path: &Path) -> Result<Value, String> {
    ...
    if bytes.len() > 16 * 1024 * 1024 {
        return Err("Hosted document exceeds 16 MiB".into());
    }
...
pub fn validate(value: &Value) -> Result<(), String> {
    let source = match text(value, "schema") {
        ...
        _ => {
            return Err(
                "Expected oi.native-source-contribution/v1 or oi.world-presentation/v1".into(),
            )
        }
    };
    ...
    validator
        .validate(value)
        .map_err(|e| format!("{}: {e}", e.instance_path))?;
    ...
    if text(value, "schema") == CONTRIBUTION_SCHEMA {
        ...
        for region in value["regions"].as_array().unwrap() {
            if !refs.insert(text(region, "region_ref")) {
                return Err("Duplicate presentation region ref".into());
            }
            for binding in region["bindings"].as_array().unwrap() {
                if !kinds.insert(text(binding, "binding_ref")) {
                    return Err("Duplicate presentation binding ref".into());
                }
            }
        }
    }
...
pub fn verify_source(manifest: &Path, value: &Value) -> Result<PathBuf, String> {
    validate(value)?;
    if text(value, "schema") != CONTRIBUTION_SCHEMA {
        return Err("Expected a native source contribution".into());
    }
    let directory = manifest
        ...
        .map_err(|e| e.to_string())?;
    let entry = directory
        .join(text(value, "entry"))
        .canonicalize()
        .map_err(|e| format!("Contribution entry unavailable: {e}"))?;
    if !entry.starts_with(&directory) {
        return Err("Contribution entry escapes its source directory".into());
    }
    let bytes = fs::read(&entry).map_err(|e| e.to_string())?;
    if digest(&bytes) != text(value, "entry_sha256") {
        return Err("Contribution entry digest differs from the reviewed manifest".into());
    }
    Ok(entry)
}
...
fn record_path(root: &Path, reference: &str) -> PathBuf {
    root.join(format!("{}.json", digest(reference.as_bytes())))
}
...
pub fn register(root: &Path, source: &Path, expected_schema: &str) -> Result<Value, String> {
    ...
    validate(&document)?;
    if text(&document, "schema") != expected_schema {
        return Err("Wrong document kind for this registry".into());
    }
    if expected_schema == CONTRIBUTION_SCHEMA {
...
pub fn show(root: &Path, reference: &str) -> Result<Value, String> {
    ...
    if digest(&bytes) != text(&record, "digest") || identity(&record["document"]) != reference {
        return Err("Registration integrity check failed".into());
    }
...
pub fn compile_registry(root: &Path, manifests: &[PathBuf]) -> Result<String, String> {
    ...
    for path in manifests {
        ...
        let relative = entry
            .strip_prefix(&root)
            .map_err(|_| "Contribution entry is outside the compilation root")?;
        let module = format!(
            "./{}",
            relative
                .with_extension("")
        ...
        for surface in document["surfaces"].as_array().unwrap() {
            if !kinds.insert(text(surface, "kind").to_owned())
                || !refs.insert(text(surface, "descriptor_ref").to_owned())
            {
                return Err("Compiled contributions duplicate a kind or descriptor ref".into());
            }
            let mut descriptor = surface.clone();
            descriptor.as_object_mut().unwrap().remove("mount_export");
            for key in ["contribution_ref", "owner", "revision"] {
                descriptor[key] = document[key].clone();
            }
